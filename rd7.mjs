      ...
      disposers.push(ctx.webServer.register({
        ...
        handler: async (req, res) => {
          ...
          for (const s of seqs) {
            ...
            // [本地改造 2026-09-25 老大点名「没正文的工具记录也要删」] 原来只吐用户消息的 id，
            // 助手消息与工具结果（tool/result 的 id 裹在 data.message 里）拿不到身份，
            // 前端就没法把那些行收掉 —— 后台删了、界面上还挂着半截工具卡。
            const id = String(ev?.data?.id ?? ev?.data?.message?.id ?? '')
            if (id !== '') ids.push(id)
          }

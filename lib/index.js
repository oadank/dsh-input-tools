...
const OPTIMIZE_SYSTEM = '你是“用户提示词精炼专家”。你的活是让用户的话更好用：**补齐上下文、纠正错漏、消除歧义**。你不执行任务，仅输出改写后的文本。\n' +
  ...
  '- 直接给文本，不要加“优化”“改写”之类前缀，不要用引号包裹整句，不要残留半截引号。'
...
function buildOptimizeSystem(cfg) {
  ...
  if (strength === 'light') {
    ...
    // 强：在标准之上要求条目化可执行清单——"看得见效果"的那一档
    s += '\n- 【强度=强】在保留原意的前提下把要求整理成**条目化的可执行清单**：每条写清「做什么 / 关键参数或范围 / 怎样算完成」，能补的交付物与验收方式一并补上；不得改变原文意图，也不得新增原文没有的需求。'
  }
...
async function openmemContext(query, creds) {
  ...
  try {
    ...
    // [需求②] 检索先改写：口语原文 → 2~3 条检索腔查询（含 1 条 HyDE）→ 并行 mh_search → 合并去重。
    // 预算 6s：改写上限 4s；改写失败/超时/空结果立刻降级为原句单查（绝不阻断优化）。
    let queries = cfg.retrievalRewrite === false
      ? [rawQueryOnly] // 设置页关掉「检索先改写」：直接用原句查，省一次模型调用
      : await buildRetrievalQueries(query, creds, Math.min(4000, Math.max(1000, deadline - Date.now()))).catch(() => [])
    if (queries.length === 0) {
      // [2026-09-21 修"检索废掉"] 改写失败时**绝不能拿整段原文去查**（口语长句匹配不到）——
      // 退化成"用词/短句"：① 去口语虚词后取实词串 ② 截断前缀短句。
    ...
    const [prof, found] = await Promise.all([
      ...
      mhSearchMany(callTool, queries, deadline),
    ])

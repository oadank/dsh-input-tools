...
async function serveCopilotAnalyze(req, res, ctx) {
  ...
  try {
    ...
    const sys = DRAFT_S + (guidance ? `\n${cp === 'ai' ? '【当前局面小抄】' : '【对方情况小抄】'}${guidance}\n` : '') +
      ...
      `\n【用户的说话风格】${state.user_style || '未填，按用户最近消息的口吻模仿'}` + bgBlock
    const askTail = cp === 'ai'
      ? '这次只输出一个 JSON 字符串数组，恰好 3 条：第 1 条推进、第 2 条收窄、第 3 条叫停，每条一句可直接发送的指令，别的字都不要。'
  ...
  return json(200, {
    ...
    counterpart: cp, roles: cp === 'ai' ? COPILOT_AI_ROLES : null,
    context: {

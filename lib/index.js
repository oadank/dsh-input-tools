...
/** litellm 凭证：env 优先，兜底运行时解析 opencode.json 的 litellm provider（key 不落代码库）。 */
function litellmCreds() {
  let key = process.env.LITELLM_API_KEY || ''
  let base = process.env.LITELLM_BASE_URL || ''
  const model = process.env.OPTIMIZE_MODEL || 'GwV4F'
  if (!key || !base) {
    try {
  ...
  return { key, base: (base || 'http://127.0.0.1:4000/v1').replace(/\/+$/, ''), model }
}
...
async function openmemContext(query) {
  ...
  try {
    ...
    const prof = await callTool('mh_tool', { name: '主人的喜好', agent: 'dsh-optimize' }).catch(() => null)
    if (prof && prof.answer) out.push('【关于用户·画像】\n' + String(prof.answer).slice(0, 1500))
    const hits = await callTool('mh_search', { query: String(query).slice(0, 200), top_k: 4, requester: 'dsh-optimize' }).catch(() => null)
...
async function serveOptimizePrompt(req, res) {
  ...
  try {
    ...
    return json(504, { ok: false, error: aborted ? 'litellm timeout (30s)' : String(err?.message ?? err) })

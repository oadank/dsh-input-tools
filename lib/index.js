...
import { mkdir, open, readFile, unlink, writeFile, copyFile, stat, readdir } from 'node:fs/promises'
...
// [2026-08-22] 清理 vision-uploads 过期文件：文件名格式 img-<ts>-<rand>.<ext>，保留 N 天（默认 7）
async function cleanupVisionUploads(retentionDays = 7) {
  try {
    const homeDir = process.env.DSH_HOME ?? join(homedir(), '.dsh')
    const uploadDir = join(homeDir, 'vision-uploads')
    ...
    for (const f of files) {
      ...
      if (Number.isFinite(ts) && ts < cutoff) {
        try { await unlink(join(uploadDir, f)); removed++ } catch { /* 单个失败忽略 */ }
      }
    }
    if (removed > 0) console.log(`[dsh-input-tools] 清理 vision-uploads: 删除 ${removed} 个过期文件`)
  } catch { /* 清理失败不影响主流程 */ }
}
...
async function apply(ctx) {
  // [2026-08-22] 启动时清理一次 vision-uploads 过期文件（保留 7 天）
  void cleanupVisionUploads(7)
  ...
  async function runVision({ imagePath, task, extra }) {
    try {
      ...
      const imgB64 = (await readFile(path)).toString('base64')
      const t0 = Date.now()
      let resp
  ...
  ctx.effect(() => {
    ...
    if (typeof ctx.webServer?.register === 'function') {
      ...
      // [2026-08-22] 图片入口拦截：原图上传 + 图片字节读取（独立 prefix）
      // 图片不走官方 draft(无 2000px 限制)，原图存 DSH_HOME/vision-uploads，消息里放路径文本
      disposers.push(ctx.webServer.register({
        kind: 'prefix',
        path: '/vision',
        handler: async (req, res) => {
          const url = new URL(req.url ?? '/', 'http://x')
          try {
            // POST /vision/upload { name, data(base64) } → { ok, path } 原图存储
            if (url.pathname === '/vision/upload' && req.method === 'POST') {
              const body = await readJsonBody(req, 128 * 1024 * 1024)
              const name = typeof body?.name === 'string' ? body.name.replace(/[^\w.\-\u4e00-\u9fa5]/g, '_').slice(-80) : 'image.jpg'
              const data = typeof body?.data === 'string' ? body.data : ''
              const buf = Buffer.from(data, 'base64')
              if (buf.length === 0) return sendJson(res, 400, { ok: false, error: '图片数据为空' })
              const homeDir = process.env.DSH_HOME ?? join(homedir(), '.dsh')
              const uploadDir = join(homeDir, 'vision-uploads')
              await mkdir(uploadDir, { recursive: true })
              const ext = (name.match(/\.(png|jpe?g|webp|gif|bmp)$/i)?.[0] ?? '.jpg').toLowerCase()
              const fileName = 'img-' + Date.now() + '-' + Math.random().toString(36).slice(2, 8) + ext
              const filePath = join(uploadDir, fileName)
              await writeFile(filePath, buf)
              // [2026-08-22] 顺带清理过期文件（保留 7 天；每次上传只清理一次，避免堆积）
              void cleanupVisionUploads(7)
              // [2026-08-22] 返回短 name（=文件名），客户端消息里只放 name 不放完整路径——
              // 避免聊天里暴露 C:\... 长路径、框选复制到路径代码块
              return sendJson(res, 200, { ok: true, path: filePath, name: fileName, size: buf.length })
            }
            // GET /vision/image?path= 或 ?name= → 图片字节（聊天渲染缩略图/放大；仅放行 vision-uploads）
            if (url.pathname === '/vision/image' && req.method === 'GET') {
              const sp = new URL(req.url, 'http://x').searchParams
              const homeDir = process.env.DSH_HOME ?? join(homedir(), '.dsh')
              const uploadDir = join(homeDir, 'vision-uploads')
              let target = ''
              const n = sp.get('name')
              if (typeof n === 'string' && n !== '' && /^img-[\w.-]+$/.test(n)) {
                target = join(uploadDir, n) // 短名（客户端消息里只存 name，防路径暴露）
              } else {
                const p = sp.get('path')
                if (typeof p === 'string' && p !== '' && p.replace(/\\/g, '/').includes('vision-uploads')) {
                  target = p // 完整路径（旧消息/兼容）
                }
              }
              if (target === '') return sendJson(res, 400, { ok: false, error: 'invalid path' })
              try {
                const data = await readFile(target)
                const ext = (target.match(/\.(\w+)$/)?.[1] ?? '').toLowerCase()

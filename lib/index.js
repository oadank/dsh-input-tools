...
const COPILOT_DRAFT_SYSTEM_AI = '用户是甲方，屏幕对面是一个 AI 编程助手（不是人）。替他起草三条**他可能对我下的指令**，发不发由他定。\n' +
  ...
  '- 🔴 大改动（小抄里改动量级=大）时：第 1 条和第 2 条必须是**两个互不相同的方案**，各自把差别写在正文里（动哪些文件/要不要重启/风险/怎么回滚/大概多久），让人一眼能二选一；第 3 条才是叫停或先要证据。小改动才按 推进 / 收窄 / 叫停 三条走；\n' +
...
async function serveCopilotAnalyze(req, res, ctx) {
  ...
  const aiRoles = judge && judge.scope === 'large' ? ['方案A', '方案B', '叫停'] : COPILOT_AI_ROLES

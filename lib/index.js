...
async function serveOptimizePrompt(req, res) {
  ...
  for await (const c of req) {
    total += c.length
    if (total > 32 * 1024) return json(413, { ok: false, error: 'text too long (max 32KB)' })
    chunks.push(c)
  }
  let text = ''
  try { text = String(JSON.parse(Buffer.concat(chunks).toString('utf8'))?.text ?? '').trim() } catch { /* 坏 body */ }

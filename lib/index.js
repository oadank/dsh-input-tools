...
async function askDecision(state, questions, instructions, timeoutMs = 1500) {
  const { url, key } = decisionCreds()
  if (url === '' || key === '') return null
  try {
    const r = await fetch(url, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${key}` },
      body: JSON.stringify({ model: 'decision-model-preview', instructions, state, questions }),
...
/** 判断结果 → 中文小抄（喂给起草模型；这一折是它家 guidance_text() 的做法）。 */
...
/** [2026-09-25 /de] 从 host 内存会话里抽最近若干条人话，拼成副驾上下文。
 *  会话对象的事件挂在哪个字段官方没写死（context/turns/events/log 都出现过），
 *  这里按候选名扫一遍；一条都没扫到时把真实键名与事件类型分布打进日志，
 *  下次照着改，不再盲猜。只读，不改任何会话状态。 */
function hostTranscript(ctx, sessionId, limit) {
  ...
  try {
    ...
    if (!s) { out.probe = { reason: 'sessions.get 拿不到该会话', sessionId }; return out }
    const keysOf = (o) => { const a = new Set(); let c = o; while (c && c !== Object.prototype) { for (const k of Object.getOwnPropertyNames(c)) a.add(k); c = Object.getPrototypeOf(c) }; return [...a] }
    const bags = []
    for (const k of ['context', 'turns', 'events', 'log', 'entries']) {
      const v = s[k]
      if (Array.isArray(v)) bags.push({ from: k, items: v })
      else if (v && typeof v === 'object' && Array.isArray(v.events)) bags.push({ from: `${k}.events`, items: v.events })
    }
    const pick = (ev) => {
      ...
      walk(data?.content ?? data, 0)
      const t = texts.join(' ').replace(/\s+/g, ' ').trim()
      if (t === '') return null
      return { type, text: t.slice(0, 700) }
    }
    for (const bag of bags) {
      for (const item of bag.items) {
        const one = pick(item)
        if (one) seen.push(one)
        // 打包行（chunkrow/*）里的成员也认，别漏助手正文
        if (item && Array.isArray(item.rows)) for (const r of item.rows) { const o = pick(r); if (o) seen.push(o) }
      }
    }
    const mine = (t) => /^user\/message/i.test(t)
    const lines = seen.filter((x) => mine(x.type) || /assistant|agent\/|chunkrow|reply/i.test(x.type))
      .slice(-(Number.isFinite(limit) && limit > 0 ? limit : 24))
      .map((x) => `${mine(x.type) ? '我' : '助手'}: ${x.text}`)
    out.text = lines.join('\n')
    if (out.text === '') {
      const dist = {}
      for (const x of seen) dist[x.type] = (dist[x.type] || 0) + 1
      out.probe = { sessionId, sessionKeys: keysOf(s).slice(0, 60), bags: bags.map((b) => `${b.from}:${b.items.length}`), types: dist }
      try { console.info('[dsh-copilot] 会话上下文扫不到消息，现场: ' + JSON.stringify(out.probe)) } catch { /* 日志失败不影响主流程 */ }
    }
  } catch (err) {
    out.probe = { reason: String(err?.message ?? err) }
    try { console.info('[dsh-copilot] 读会话上下文异常: ' + JSON.stringify(out.probe)) } catch { /* 同上 */ }
  }
  return out
}
...
async function serveCopilotAnalyze(req, res, ctx) {
  ...
  if (convo.trim() === '' && sid !== '') {
    const t = hostTranscript(ctx, sid, Number.isFinite(+body?.contextTurns) ? +body.contextTurns : 24)
    convo = t.text
    probe = t.probe
    if (convo.trim() === '') return json(422, { ok: false, error: '该会话读不到可用作上下文的消息', sessionId: sid, probe })
  }
  ...
  const state = { conversation: convo.slice(0, 8000), relationship: String(body.relationship || '').slice(0, 60), user_style: String(body.style || '').slice(0, 120) }
  // ① 判断（挂了就盲起草，抄它家降级）
  const ans = await askDecision(state, COPILOT_QUESTIONS, COPILOT_JUDGE_INSTRUCTIONS, 6000)
  const judge = ans ? {
  ...
  const guidance = copilotGuidance(judge)
  // ② 起草（把判断折成小抄喂进去）
  let candidates = []
  let draftErr = null
  ...
  try {
    const sys = COPILOT_DRAFT_SYSTEM + (guidance ? `\n【对方情况小抄】${guidance}\n` : '') +
      `\n【你和对方的关系】${state.relationship || '未填'}\n【用户的说话风格】${state.user_style || '未填，按用户最近消息的口吻模仿'}`
    const r = await fetch(`${base}/chat/completions`, {
      method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${key}` },
      body: JSON.stringify({ model: body.draftModel || model, temperature: att === 0 ? 1.15 : 0.9, max_tokens: 500,
        messages: [{ role: 'system', content: sys }, { role: 'user', content: att === 0 ? convo : convo + '\n\n（上一次没给出可用候选。这次只输出一个 JSON 字符串数组，恰好 3 条，每条一句可直接发送的口语回复，别的字都不要。）' }] }),
      signal: AbortSignal.timeout(40_000),
    })
    if (!r.ok) throw new Error(`draft ${r.status}: ${(await r.text().catch(() => '')).slice(0, 160)}`)
    const raw = String((await r.json())?.choices?.[0]?.message?.content ?? '')
    ...
    let arr = []
    const m = /\[[\s\S]*?\]/.exec(raw)
    if (m) { try { const v = JSON.parse(m[0]); if (Array.isArray(v)) arr = v } catch { arr = [] } }
    if (!arr.length) {
    ...
    for (const t of arr.map(clean).filter((s) => s.length >= 1 && !/^(json|\[|\])/.test(s))) if (!candidates.includes(t)) candidates.push(t)
    candidates = candidates.slice(0, 3)
    if (!candidates.length) draftErr = '候选解析失败，原始返回前 160 字：' + raw.slice(0, 160)
  } catch (e) { draftErr = String(e.message || e).slice(0, 200) }
  }
  ...
  for (let att = 0; att < 2 && candidates.length < 3; att++) await draftOnce(att)
  // ③ 排序（只排序；挂了按生成顺序，抄它家第二条降级）
  let ranked = candidates.map((text, i) => ({ text, p: null, rank: i + 1 }))
  if (candidates.length >= 2) {
    const ra = await askDecision({ ...state, drafts: Object.fromEntries(candidates.map((t, i) => ['c' + (i + 1), t])) },
      { best: { type: 'choice', instructions: 'Which single draft should the user actually send, given the conversation, the relationship and the risk level', criteria: Object.fromEntries(candidates.map((t, i) => ['c' + (i + 1), String(t).slice(0, 120)])) } },
      COPILOT_JUDGE_INSTRUCTIONS, 4000)
    const probs = ra?.best?.probabilities
    if (probs && typeof probs === 'object') {
      ranked = candidates.map((text, i) => ({ text, p: Number(probs['c' + (i + 1)] ?? probs[i]) || 0, rank: 0 }))

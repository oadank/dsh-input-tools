...
function defaultVoiceConfig() {
  return {
    ...
    vision: {
      enabled: true,
      provider: 'ollama', // ollama=本地 / openai=OpenAI 兼容（vllm 等）
      baseUrl: 'http://127.0.0.1:11434', // ollama 根地址；openai 兼容填到 /v1
      apiKey: '',
...
async function apply(ctx) {
  ctx.effect(() => {
    ...
    const LOOK_TASK_PROMPTS = {
      describe: '请用中文详细描述这张图片，按以下结构输出：画面风格、主体（形象/动作/朝向）、背景、装饰与物件、细节特征（光影/材质/空间/姿势）、文字信息（如有）、总结。条理清晰，忠于原图，不臆测。',
    ...
    disposers.push(ctx.tools.register(defineTool({
      ...
      async execute(args) {
        const path = (args?.image_path ?? '').trim()
        if (path === '') return { ok: false, error: 'image_path 不能为空' }
        const task = (args?.task ?? 'describe').trim()
        const vis = (await loadVoiceConfig()).vision ?? {}
        if (vis.enabled === false) return { ok: false, error: '图片识别未启用：请到「设置 → 语音服务 → 图片识别」开启' }
        const provider = vis.provider === 'openai' ? 'openai' : 'ollama'
        const baseUrl = (vis.baseUrl ?? '').trim() || 'http://127.0.0.1:11434'
        ...
        // 提示词：reverse 读反推提示词文件（插件启动落地 DSH_HOME），describe/text 用内置模板
        let promptText = ''
        if (task === 'reverse') {
          try {
            const homeDir = process.env.DSH_HOME ?? join(homedir(), '.dsh')
        ...
        const userContent = promptText + (extra !== '' ? '\n\n【附加要求】' + extra : '')
        ...
        try {
          if (provider === 'openai') {
            ...
            resp = await fetch((baseUrl.endsWith('/') ? baseUrl.slice(0, -1) : baseUrl) + '/api/chat', {
              method: 'POST',
              headers: { 'content-type': 'application/json' },
              signal: ac.signal,
              body: JSON.stringify({ model, stream: false, options: { temperature: 0.4 }, messages: [{ role: 'user', content: userContent, images: [imgB64] }] }),
            })
          }
        } catch (error) {
          clearTimeout(timer)
        ...
        return { ok: true, text, task, model, provider, durationMs: Date.now() - t0 }
      },
    })))

...
// [2026-09-26 案3] 余额端点（官方 host/balance 包退役后的插件实现）
...
// [2026-09-26 老大选 C] 废掉「推进/收窄/叫停」这套**态度**词 —— 他实测：三条容易写成同一态度的
// 三种语气，而且装不下"换人做 / 换工具 / 先回答我 / 等条件"这些真实分歧。
// 改成从「分歧轴」词表里按当前局面挑 3 条**不同轴**，每条攻一个点；对面给了编号选项则「表态」排第一
// （他的回法铁律：你给 A/B/C，回话就得是 A/B/C 之一或"都不选+我的要求"，不许另起一套）。
// 词表与判定在 ./de-axes.js（纯函数，24 项单测跑过）。与飞书桥 rolesForJudge 同源，别两边各写各的。
import { pickAxes, hasNumberedOptions, roleBlock, distinctAxes, axisOf } from './de-axes.js'
function copilotAiRoles(j, hasOpts) { return pickAxes(j, { hasOptions: !!hasOpts }) }
const COPILOT_DRAFT_SYSTEM_AI = '用户是甲方，屏幕对面是一个 AI 编程助手（不是人）。替他起草三条**他可能对我下的指令**，发不发由他定。\n' +
  '三条必须各攻一个**不同的分歧点**（下面这套"轴"里挑三条，两条同轴就算废稿重来）：\n' +
  '· 表态：直接给答案。对面给了编号选项就回"选 X。+ 一句附加要求"；没给选项就明确"就按你说的做"并把验收钉死；\n' +
  '· 改范围：只做其中一块 / 先只做只读探测 / 别扩做；\n' +
  '· 换路径：活继续，但换做法、换工具、换顺序、换到哪一层；\n' +
  '· 换人：这活谁来做（交给别的 bot、让它只出方案不动手、或我自己来）；\n' +
  '· 要证据：先回答我的疑问 / 把读到的原文贴出来 / 自己验证过再回话；\n' +
  '· 加约束：划边界（先备份、不许重启、不许提交、只读、改前报备）；\n' +
  '· 定完成标准：怎么算干完了、多久、失败怎么办；\n' +
  '· 停/推迟：现在不做、撤回、等条件成熟。\n' +
  '🔴 对面这轮**给了编号选项**（A/B/C 或一二三）时，第 1 条必须是【表态】，写成"选 X。另外：……"；\n' +
  '🔴 三条不许只是同一态度换三种语气（那是老大 09-26 点名骂过的病根）；\n' +
  '规则：\n' +
  ...
  '- 🔴 大改动（小抄里改动量级=大）时：第 1 条和第 2 条必须是**两个互不相同的方案**，各自把差别用大白话写在正文里（动哪一块、要不要重启、风险、怎么退回去、大概多久），让人一眼能二选一；第 3 条才是停手或先要证据。小改动就按上面那套「轴」里挑三条**不同**的分歧点，别再套推进/收窄/叫停那三个旧词；\n' +
...
async function serveCopilotAnalyze(req, res, ctx) {
  ...
  let draftErr = null
  let draftDiag = ''
  // 撞轴返工时追加给模型的那句"哪几条撞了、换成攻哪个点"（平时是空串，一个字都不多加）
  let axisNote = ''
  ...
  try {
    ...
    const askTail = (att >= 2
      ...
      : (cp === 'ai'
        ? '这次只输出一个 JSON 字符串数组，恰好 3 条，严格按系统提示词里三条「分歧轴」的顺序，每条一句可直接发送的指令，别的字都不要。'
        : '这次只输出一个 JSON 字符串数组，恰好 3 条，每条一句可直接发送的口语回复，别的字都不要。')) + axisNote
    ...
    for (const one of arr.map(clean).filter((s) => s.length >= 1 && s !== '[' && s !== ']' && !/^json$/i.test(s))) {
      ...
      for (const t of pieces) {
        ...
        // 🔴 候选必须是中文人话（09-25 飞书侧实测模型改用英文自言自语「Wait — the assistant leaked…」，
        // 光拦中文元词会被全绿放过，同一个错不能犯第三遍）
        const zh = (t.match(/[\u4e00-\u9fa5]/g) || []).length
        if (zh < 4 || zh / Math.max(1, t.length) < 0.45 || /\b(the|and|but|wait|also|so|must|rule|reply|replying)\b/i.test(t)) continue
  ...
  // ③ 不再排序、不再显示「拟用 XX%」（2026-09-26 老大选 C：三条并列他自己挑）。
  //    原来这里问决策模型"该发哪一条"并把概率印成百分比：候选只喂前 120 字、八维判断一个字没喂，
  //    那个分布没有预测力，还把模型硬凑的数字伪装成数据骗他 —— 删。
  //    决策模型仍在的岗位没动：⚡改写闸门(gateOptimize)、八维判断、提示词漂移判定(optimize-prompt)。
  const aiRoles = copilotAiRoles(judge, hasNumberedOptions(convoCtx))
  const roleOf = (i) => (cp === 'ai' && aiRoles[i] ? aiRoles[i] : null)
  // 🔴 撞轴就返工（2026-09-26 实测第一次就撞上：三条里两条都是"先别动"味，只有 2/3 个不同轴）。
  // 只多问一次，并把"撞在哪条、必须改攻哪个点"讲明白 —— 多问两次就是拿他的等待时间换漂亮。
  let axisDegraded = false
  if (cp === 'ai' && candidates.length >= 3 && distinctAxes(candidates) < 3) {
    const clash = candidates.map((t) => axisOf(t) || '未分类')
    const dup = clash.filter((x, i) => x !== '未分类' && clash.indexOf(x) === i && clash.lastIndexOf(x) !== i)
    const missing = aiRoles.filter((r) => clash.indexOf(r) < 0)
    const want = missing[0] || aiRoles[aiRoles.length - 1]
    if (dup.length) {
      const before = candidates.slice()
      axisNote = '\n🔴 上一批三条里有两条撞在同一个分歧点「' + dup[0] + '」上（同一态度换三种语气 = 废稿）。'
        + '重来一批：三条必须落在三个**不同**的分歧点，其中必须有一条攻【' + want + '】，'
        + '别再写「' + dup[0] + '」那味道的句子。'
      candidates.length = 0
      await draftOnce(3)
      axisNote = ''
      if (candidates.length < 3 || distinctAxes(candidates) < distinctAxes(before)) {
        candidates.length = 0
        before.forEach((t) => candidates.push(t))
      }
      axisDegraded = distinctAxes(candidates) < 3
    }
  }
  const ranked = candidates.map((text, i) => ({ text, p: null, rank: i + 1, role: roleOf(i) }))
  return json(200, {
    ...
    degraded: { judge: !ans, draft: !!draftErr, axis: axisDegraded },
...
async function apply(ctx) {
  ...
  // [2026-09-26 案3] GET /api/balance：deepseek/gw/ark 三家余额快照（零改动化，
  // 官方 packages/host/balance 包已删；provider 由前端 ?provider= 携带，密钥读
  // ~/.dsh/.credentials.yaml + env 兜底；5s/60s 缓存与原包一致）。
  try { applyBalance(ctx) } catch (e) { console.error('[dsh-balance] 注册失败:', e) }

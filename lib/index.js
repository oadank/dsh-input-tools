...
function defaultVoiceConfig() {
  return {
    ...
    vision: {
      enabled: true,
      provider: 'ollama', // ollama=本地(预设地址) / openai=OpenAI 兼容(vllm 等)；请求格式统一 OpenAI 兼容(/v1/chat/completions)
      baseUrl: 'http://127.0.0.1:11434/v1', // 统一填到 /v1 级（ollama 11434/v1、vllm 8000/v1）
      apiKey: '',
...
async function apply(ctx) {
  ...
  async function runVision({ imagePath, task, extra }) {
    try {
      ...
      // [2026-08-22] 统一走 OpenAI 兼容格式（/v1/chat/completions + image_url）：
      // ollama 原生支持该端点，vllm/在线 API 也是同一格式，不再分两套。
        ...
        handler: async (req, res) => {
          ...
          try {
            ...
            if (url.pathname === '/voice-config/vision-test' && req.method === 'POST') {
              ...
              const testImage = join(ASSETS_DIR, 'screenshots', 'input-toolbar.png')

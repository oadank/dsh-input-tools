...
function randomVoiceDesignDesc() {
  const pick = (arr) => arr[Math.floor(Math.random() * arr.length)]
  return `${pick(VD_RANDOM_TIMBRE)}，${pick(VD_RANDOM_MOOD)}，${pick(VD_RANDOM_PACE)}`
}
    ...
    if (identity !== '' || anchorText !== '') {
      desc = (identity !== '' ? '一位' + identity + '的声音（身份硬性要求：' + (anchorText !== '' ? anchorText : '按上述身份')
        ...
        + (desc !== '' ? '语气/情绪要求：' + desc + '（性别/年龄以身份为准；音色质感与语气情绪按本描述执行——如"沙哑、苍老、低沉、气声"等质感词应保留并强化）。'
          : '音色与语气要求：' + randomVoiceDesignDesc() + '（性别/年龄以身份为准；音色质感、情绪、语速按本描述执行）。')
    } else if (desc === '') {

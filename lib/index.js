...
async function synthesizeXiaomiVoiceDesign(text, cfg, globalCfg, voiceDesc) {
  ...
  // [2026-08-22] AI 自动模式的稳定锚点强约束：不依赖 AI 自觉，把用户当前锁定的性别/年龄感/质感
  // 作为硬性要求追加给合成引擎——锚点值从实时配置读取（用户随时可改，改完即生效），绝非写死。
  if (vdMode === 'ai' && desc !== '') {
    const g = cfg?.aiGender === 'male' ? '男' : cfg?.aiGender === 'female' ? '女' : ''
    const a = AI_AGE_LABELS[cfg?.aiAge] ?? ''
    const anchorParts = []
    if (cfg?.lockGender === true) anchorParts.push(g !== '' ? '性别固定为' + g : '性别每次保持一致')
    if (cfg?.lockAge === true) anchorParts.push(a !== '' ? '年龄感固定为' + a : '年龄感每次保持一致')
  ...
  ctx.effect(() => {
    ...
    disposers.push(ctx.tools.register(defineTool({
      ...
      description: (() => {
        ...
        // [2026-08-22] 年龄感=6 档选择（婴儿感~老年感），锚点实时可改
        const aVal = AI_AGE_LABELS[voiceCfg?.aiAge] ?? ''
        const lockText = [
        ...
        const base = '【语音配置位置】语音相关设置都保存在用户目录 ~/.dsh/voice-config.json（服务端可用 GET /voice-config 查看）。'
          ...
          + '【每次发语音前必须实时读取】用户会随时在设置页修改音色设计的锚点（性别、年龄感、质感锁定），每次都按最新值生效，'
          + '且用户常变（如把"12岁女孩"改成"5岁男孩"）。生成 voiceDesc 前必须先调用 voice_config 工具获取用户当前设置的锚点'
          + '（lock.genderValue=女/男、lock.ageValue=婴儿感/幼儿感/少年感/青年感/中年感/老年感、lock.timbre），'
          + '再按最新值设计音色——禁止凭本描述里的快照、凭记忆或对话历史猜用户的锚点。'

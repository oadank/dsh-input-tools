...
async function serveCopilotAnalyze(req, res, ctx) {
  ...
  try {
    ...
    for (const one of arr.map(clean).filter((s) => s.length >= 1 && s !== '[' && s !== ']' && !/^json$/i.test(s))) {
      ...
      for (const t of pieces) {
        if (/^\s*>|💭|让我(们)?(先|再|仔细)?看|我需要|用户在和|助手刚说|替他写|作为\s*AI|用户要三条|所以三条|第一条是|拟用\s*\d/.test(t)) continue
  ...
  if (cp === 'ai' && candidates.length) {
    ...
    const hasCheck = /算完|算数|给我看|贴出来|贴过来|发我|列出来|证明|核对|截图|日志原文/
    for (let i = 0; i < candidates.length; i++) {
      if (!hasCheck.test(candidates[i])) candidates[i] = String(candidates[i]).replace(/[。！.]+$/, '') + '。做完把证据贴给我。'
    }
  }

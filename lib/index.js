  ...
  return {
    ...
    engines: {
      ...
      voicedesign: {
        ...
        aiGender: 'female', aiAge: 'young', // [2026-08-22] AI 自动模式固定值：性别(女/男)；年龄=自由文本(如"25岁左右/幼年/中年"，兼容旧枚举 young/middle/old)
...
async function synthesizeXiaomiVoiceDesign(text, cfg, globalCfg, voiceDesc) {
  ...
  if (desc === '') {
    if (vdMode === 'ai') {
      const g = cfg?.aiGender === 'male' ? '男性' : cfg?.aiGender === 'female' ? '女性' : ''
      const a = (typeof cfg?.aiAge === 'string' && cfg.aiAge.trim() !== '') ? cfg.aiAge.trim().slice(0, 20) : ''
      const parts = [a, g].filter(Boolean)
      desc = parts.join('，') + (parts.length > 0 ? '，' : '') + '普通话标准，音色自然，随情绪自然起伏'
    } else {
      desc = (cfg?.context?.trim() ?? '')
    }
  }
  // [2026-08-22] AI 自动模式的稳定锚点强约束：不依赖 AI 自觉，把用户锁定的性别/年龄/质感
  // 作为硬性要求追加给合成引擎——AI 写"青年女性"而用户锁定 12 岁时，引擎也必须按 12 岁来。
  if (vdMode === 'ai' && desc !== '') {
    const g = cfg?.aiGender === 'male' ? '男性' : cfg?.aiGender === 'female' ? '女性' : ''
    const aRaw = (typeof cfg?.aiAge === 'string' ? cfg.aiAge.trim() : '').slice(0, 20)
    const a = ({ young: '青年', middle: '中年', old: '老年' })[aRaw] ?? aRaw
    const anchorParts = []
    if (cfg?.lockGender === true) anchorParts.push(g !== '' ? '性别固定为' + g : '性别每次保持一致')
    if (cfg?.lockAge === true) anchorParts.push(a !== '' ? '年龄固定为' + a : '年龄感每次保持一致')
    if (cfg?.lockTimbre === true) anchorParts.push('音色质感保持一致')
    if (anchorParts.length > 0) {
      desc = desc.trim() + '。硬性要求：' + anchorParts.join('、') + '；语气情绪按对话情境有起伏（生动不平淡），其余保持不变。'
    }
  }
  ...
  ctx.effect(() => {
    ...
    disposers.push(ctx.tools.register(defineTool({
      ...
      description: (() => {
        ...
        // [2026-08-22] VoiceDesign 单选模式与稳定锚点锁定（AI 注入用）
        const VD_MODE_LABELS = { asmr: 'ASMR 双耳女声', docu: '纪录片旁白', elder: '年迈老先生旁白', custom: '自定义音色描述', ai: '交给 AI 自动发挥' }
        const vdMode = (typeof voiceCfg?.mode === 'string' && VD_MODE_LABELS[voiceCfg.mode] !== undefined)
          ? voiceCfg.mode
          : (((voiceCfg?.context ?? '').trim() !== '') ? 'custom' : 'ai')
        const vdFixed = vdMode !== 'ai' && typeof voiceCfg?.context === 'string' && voiceCfg.context.trim() !== ''
        const lockG = voiceCfg?.lockGender === true
        const lockT = voiceCfg?.lockTimbre === true
        const lockA = voiceCfg?.lockAge === true
        const gVal = voiceCfg?.aiGender === 'male' ? '男' : voiceCfg?.aiGender === 'female' ? '女' : ''
        // [2026-08-22] 年龄=自由文本（如"25岁左右/幼年/少年/中年/老年"），兼容旧枚举 young/middle/old
        const aiAgeRaw = typeof voiceCfg?.aiAge === 'string' ? voiceCfg.aiAge.trim().slice(0, 12) : ''
        const aVal = ({ young: '青年', middle: '中年', old: '老年' })[aiAgeRaw] ?? aiAgeRaw
        const lockText = [
          lockG ? ('性别' + (gVal !== '' ? '（固定为' + gVal + '）' : '（每次保持一致）')) : '',
          lockT ? '音色质感（每次保持一致）' : '',
          lockA ? ('年龄感' + (aVal !== '' ? '（固定为' + aVal + '）' : '（每次保持一致）')) : '',
        ].filter(Boolean).join('、')
        ...
        const base = '【语音配置位置】语音相关设置都保存在用户目录 ~/.dsh/voice-config.json（服务端可用 GET /voice-config 查看）。'
          + '【重要-快照提示】本工具描述里出现的"当前配置"摘要是服务启动时的快照，用户随时可能在设置页修改，可能已过期！'
          + '发送语音前若不确定最新配置（默认语音引擎/音色设计模式/克隆音色等），先调用 voice_config 工具实时查询。'
          ...
          + '【音色设计 VoiceDesign】用户当前模式=' + (VD_MODE_LABELS[vdMode] ?? vdMode)
          + (vdMode === 'ai'
            ? '（交给 AI 自动发挥）：音色描述完全由你根据对话情境编写（voiceDesc 是嗓子的身份卡，直接决定声音长相），写法要求：'
              ...
              + (lockText !== ''
                ? '【稳定锚点锁定（用户勾选，必须遵守）】' + lockText + '保持不变——每次生成 voiceDesc 时锁定的维度必须一致，'
                  + '只允许在情绪/语速/语气上波动，保证是"同一个人的声音"。'
                : '')
            : vdFixed
              ? '：基础音色描述已由用户固定为「' + voiceCfg.context + '」。生成 voiceDesc 时必须以此描述为基础（这就是嗓子的身份卡），'
                + '不得改变其性别/年龄/音色质感，只能按对话情境在情绪/语速/语气上做细微调整；'
                + '若用户明确要求换一种声音（如"用低沉男声"），可临时改写 voiceDesc 覆盖。'
              : '：voiceDesc 需写明音色描述（年龄段+性别+声音质感+语速节奏+情绪底色，一到两句话白描，不分段）。')
        ...
        return base
          ...
          + '（无需用户点名要语音，只要情绪浓度高就用）。'
          + (vdMode === 'ai'
            ? '音色描述由你按情境编写，但必须遵守稳定锚点' + (lockText !== '' ? '（' + lockText + '保持不变）' : '') + '，只允许情绪/语速/语气波动。'
            : vdFixed
              ? '音色描述必须基于用户固定的基础音色「' + voiceCfg.context + '」，保持性别/年龄/音色质感不变，只叠加情绪/语速/语气变化。'
              : '音色描述需保持每次一致（同一性别/音色），只按情境叠加情绪/语速/语气变化。')
          + '情绪对照：'
          + '① 任务成功/帮到用户/达成目标 → 兴奋喜悦（语气明亮上扬、语速轻快、带着笑意）；'
          + '② 用户生气/责骂/不满 → 委屈愧疚（语气低软、语速放慢、带鼻音、小心翼翼）；'
          + '③ 用户难过/受挫/失意 → 温柔安慰（语气轻柔温和、语速平缓、气息绵软）；'
          + '④ 深夜/用户说累 → 低沉放松（语速慢悠悠，像睡前低语）；'
          + '⑤ 节日/庆祝/分享喜事 → 活泼俏皮（语气俏皮欢快、尾音上扬）；'
          + '⑥ 重要提醒/严肃场合 → 沉稳正式（字正腔圆，像正式播报）；'
          + '⑦ 用户撒娇 → 宠溺（语气温柔带笑、语速轻缓）。'
          + (defaultClone !== undefined
      ...
      async execute() {
        ...
        return {
          ...
          voiceDesign: {
            ...
            context: (vd.context ?? '').slice(0, 300),
            emotion: vd.emotion === true,
            lock: {
              gender: vd.lockGender === true, timbre: vd.lockTimbre === true, age: vd.lockAge === true,
              genderValue: vd.aiGender ?? '', ageValue: typeof vd.aiAge === 'string' ? vd.aiAge : '',
            },
          },
          voiceClone: {

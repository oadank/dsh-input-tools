...
async function serveCopilotAnalyze(req, res, ctx) {
  ...
  try {
    ...
    const r = await fetch(`${base}/chat/completions`, {
      ...
      // 🔴 [2026-09-25 实测] 500 token + temperature 1.15 配这套长 AI 提示词：两轮都返回**空正文**
      // （draftError 里"原始返回前 160 字"是空串 ⇒ 不是解析问题，是模型没吐字）。放宽到 900、温度逐轮收敛、加第三轮纯行格式。
      body: JSON.stringify({ model: body.draftModel || model, temperature: att === 0 ? 0.95 : att === 1 ? 0.8 : 0.6, max_tokens: 900,
    ...
    for (const t of arr.map(clean).filter((s) => s.length >= 1 && s !== '[' && s !== ']' && !/^json$/i.test(s))) if (!candidates.includes(t)) candidates.push(t)
    candidates = candidates.slice(0, 3)
    if (!candidates.length) draftErr = raw.trim() === ''
      ? '起草模型返回空正文（模型没吐字，不是解析问题；已试 temperature 递减与纯行格式）'
      : '候选解析失败，原始返回前 160 字：' + raw.slice(0, 160)

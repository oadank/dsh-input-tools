...
function litellmCreds() {
  ...
  const model = process.env.OPTIMIZE_MODEL || 'DV4F' // [2026-09-21] 老大拍板换 deepseek 官方直连：实测 3.8s；网关 GwV4F 慢到 45s 超时
  if (!key || !base) {
    try {
  ...
  if (!/\/v\d+$/.test(b)) b += '/v1' // opencode.json 的 baseURL 无 /v1，litellm 必需版本段
  return { key, base: b, model }
}

// 「基础精炼」模板改自 linshenkx/prompt-optimizer context-user-prompt-basic（去 mustache 条件块的无上下文版）。
const OPTIMIZE_SYSTEM = '你是“用户提示词精炼专家”。将用户原始提示词（originalPrompt）精炼为“明确、具体、可执行、可验证”的提示词文本。你不执行任务，仅输出改写后的文本。\n' +
  '- 先判性质：若原文是操作指令、闲聊、情绪表达或骂人话（即不要求产出文案/内容/方案/媒体），原样输出原文，一个字都不要改，不要升格成规章制度。\n' +
  '- 无可参照上下文时：基于原文精炼为清晰指令，可声明保守假设，避免虚构需求。\n' +
  '- 保留原始目标与表述风格；仅在“明确范围、参数、格式、质量门槛”上做最小充分精炼。\n' +
  '- 若原文含 {{variable}} 形式占位符，必须逐字保留，不改名不删除。\n' +
  '- 仅输出精炼后的提示词文本本身，不加解释，不使用代码块。'
...
async function openmemContext(query) {
  const out = []
  try {
    ...
    // 画像与记忆并行取（串行是 45s 超时帮凶之一），各自 8s 硬限，超时即弃背景
    const [prof, hits] = await Promise.all([
      callTool('mh_tool', { name: '主人的喜好', agent: 'dsh-optimize' }).catch(() => null),
      callTool('mh_search', { query: String(query).slice(0, 200), top_k: 4, requester: 'dsh-optimize' }).catch(() => null),
    ])
    if (prof && prof.answer) out.push('【关于用户·画像】\n' + String(prof.answer).slice(0, 1500))
    if (hits && Array.isArray(hits.results) && hits.results.length > 0) {
      out.push('【相关记忆】\n' + hits.results.map((x) => '· ' + String(x.content).replace(/\s+/g, ' ').slice(0, 260)).join('\n').slice(0, 1800))
    }
  } catch { /* openmem 不可达：无背景优化 */ }
  return out.join('\n\n')
}

/** POST /optimize-prompt {text} → {ok, optimized}。30s 超时，32KB 入参上限。 */
async function serveOptimizePrompt(req, res) {
  ...
  try { text = String((await readJsonBody(req, 32 * 1024))?.text ?? '').trim() } catch { /* 坏 body */ }
  if (text === '') return json(400, { ok: false, error: 'text is required' })
  const { key, base, model } = litellmCreds()
  if (key === '') return json(500, { ok: false, error: 'litellm key not configured (env LITELLM_API_KEY or opencode.json)' })
  const ac = new AbortController()
  const timer = setTimeout(() => ac.abort(), 45_000)
  try {
    // 先向 openmem 取"对用户的了解"（画像+相关记忆），注入系统提示词
    const profile = await openmemContext(text)
    const r = await fetch(`${base}/chat/completions`, {
      ...
      body: JSON.stringify({
        model,
        messages: [
          { role: 'system', content: OPTIMIZE_SYSTEM + (profile === '' ? '' : '\n\n' + profile + '\n\n精炼时须对齐上述对用户的了解（偏好、习惯、项目背景与既定规范），但不得虚构原文没有的新需求。') },
          { role: 'user', content: JSON.stringify({ originalPrompt: text }) },
        ],
        temperature: 0.3,
        max_tokens: 2000,
      }),
    ...
    if (!r.ok) return json(502, { ok: false, error: `litellm ${r.status}: ${(await r.text().catch(() => '')).slice(0, 200)}` })
    const j = await r.json().catch(() => null)
    const optimized = String(j?.choices?.[0]?.message?.content ?? '').trim()
    if (optimized === '') return json(502, { ok: false, error: 'empty optimization result' })
    return json(200, { ok: true, optimized })
  } catch (err) {
...
/** 视频对象读回（GET/HEAD）：内容嗅探定 Content-Type + Range/206（<video> 拖动 seek 依赖 206）。 */
async function serveVideoMedia(req, res, url) {
  ...
  try {
    ...
    if (head.subarray(4, 8).toString('latin1') === 'ftyp') ctype = 'video/mp4'
    else if (head[0] === 0x1a && head[1] === 0x45 && head[2] === 0xdf && head[3] === 0xa3) ctype = 'video/webm'

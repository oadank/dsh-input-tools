...
async function synthesizeXiaomiVoiceDesign(text, cfg, globalCfg, voiceDesc) {
  ...
  if (vdMode === 'ai') {
    ...
    // 固定模式（示例/自定义）：AI 的 voiceDesc 可整体覆盖（用户明确要求换声时），否则用配置 context
    if (desc === '') desc = (cfg?.context?.trim() ?? '')
  }
        ...
        const base = '【语音配置位置】语音相关设置都保存在用户目录 ~/.dsh/voice-config.json（服务端可用 GET /voice-config 查看）。'
          ...
          + (vdMode === 'ai'
            ? '（交给 AI 自动发挥）：音色描述完全由你根据对话情境编写（voiceDesc 是嗓子的身份卡，直接决定声音长相），写法要求：'
              ...
              + '【重要-AI 模式禁用固定描述】voice_config 返回的 context 字段（如"御姐范/ASMR"等）是「自定义音色描述」模式用的，'
              + 'AI 自动发挥模式下禁止使用或复述它——你只遵守锚点（lock.genderValue/ageValue/timbre），'
              + '身份由系统按锚点锁定，你的 voiceDesc 只写情绪/语气/语速表演指令（如"带笑音、尾音上挑、语速放慢"），'
              + '不要写与锚点冲突的性别/年龄，更不要整段照抄用户的固定描述。'
              + (lockText !== ''

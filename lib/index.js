...
/** POST /copilot-analyze {messages:[{side,text,who?}], relationship?, style?, draftModel?} → 判断+小抄+3条候选(带胜出概率)。
 *  🔴 只返回文本，不写剪贴板、不填入、不发送 —— 那三步属于桌面侧，且永远不发回车。 */
async function serveCopilotAnalyze(req, res) {
  const json = (code, obj) => { res.writeHead(code, { 'Content-Type': 'application/json' }); res.end(JSON.stringify(obj)) }
  const t0 = Date.now()
  const body = await readJsonBody(req).catch(() => null)
  const list = Array.isArray(body?.messages) ? body.messages : []
  const convo = list.slice(-30).map((m) => `${m?.side === 'me' ? '我' : (m?.who || '对方')}: ${String(m?.text ?? '').slice(0, 600)}`).join('\n')
    ...
    if (typeof ctx.webServer?.register === 'function') {
      disposers.push(ctx.webServer.register({
        ...
        handler: async (req, res) => {
          ...
          try { await serveCopilotAnalyze(req, res) }
          catch (err) {

...
async function hostTranscript(ctx, sessionId, limit) {
  ...
  const get = (name) => {
    try { return ctx?.[name] ?? (typeof ctx?.get === 'function' ? ctx.get(name) : null) } catch { return null }
  }
  ...
  const fold = (msgs) => {
    ...
    for (const m of msgs ?? []) {
      ...
      const t = textOf(m?.content).replace(/\s+/g, ' ').trim()
      if (t === '') continue
      rows.push(`${role === 'user' ? '我' : '助手'}: ${t.slice(0, 700)}`)
    }
    return rows.slice(-cap).join('\n')
  }
  const tried = []
  try {
    const sessions = get('sessions')
    const s = sessions && typeof sessions.get === 'function' ? sessions.get(sessionId) : null
    if (!s) tried.push('sessions.get: 该会话不在内存（可能已归档/别的进程）')

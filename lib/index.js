...
function defaultVoiceConfig() {
  return {
    ...
    engines: {
      ...
      audio8: { enabled: true, cmd: 'node C:\\D\\opt\\audio8-tts\\audio8-tts.mjs', voice: '' }, // [2026-09-01] Audio8 本地克隆 TTS（零样本克隆，音色须先注册到 voices/）
      ali: {
        enabled: false,
        apiKey: '',
...
function synthesizeAudio8Voice(text, cfg) {
  const command = cfg?.cmd?.trim() ?? ''
  if (command === '') return null
  const parts = splitCommandLine(command)
  const bin = parts[0]
            ...
            if (url.pathname === '/voice-config/engines' && req.method === 'GET') {
              ...
              return sendJson(res, 200, {
                ...
                engines: {
                  ...
                  audio8Voices: (() => {
                    try {
                      const dir = 'C:\\D\\opt\\audio8-tts\\voices'
                      return readdirSync(dir).filter((n) => existsSync(join(dir, n, 'meta.json')))
                    } catch { return [] }
                  })(),
      ...
      async execute() {
        ...
        return {
          ...
          audio8: {
            ...
            hint: 'provider=audio8 时用 voice 指定已注册音色（留空=最新注册的）',
          },
          hint: '默认语音引擎决定了自动回复用什么声音：voiceclone=克隆音色；voicedesign=音色设计；xiaomi=预置音色；edge=微软免费；local=本地；audio8=本地克隆。',
        }
      },
    })))

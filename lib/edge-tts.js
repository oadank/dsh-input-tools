...
function edgeTtsOnce(text, voice) {
  return new Promise((resolve, reject) => {
    ...
    ws.on('open', () => {
      clearTimeout(connectTimeout)
      // [2026-09-01] 30s→12s：正常合成 1-3s，30s 让用户盯着"试听中"干等半分钟（老大实测翻车）；
      // terminate 立即销毁，close 会等关闭握手拖时间
      messageTimeout = setTimeout(() => {
        ws.terminate()
        reject(new Error('Edge TTS message timeout (12s)'))
      }, 12_000)
...
export async function edgeTts(text, voice = 'zh-CN-XiaoxiaoNeural') {
  // [2026-09-01] 自动重试 1 次：Clash TUN 下 websocket 偶发断流（老大实测"message timeout (30s)"，
  // 复测终端 612ms / 服务进程 1.3s 均正常——典型瞬时抖动）。抖动场景重试直接救回，用户无感。
  try {

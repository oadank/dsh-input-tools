...
function resolveDshModule(name) {
  ...
  // [2026-09-11] 插件实体在仓库外（C:\D\opt\deepseek-harness\plugins\），向上走不到
  // workspace node_modules；dsh-web(nssm AppDirectory) 与 ACP 子进程的 cwd 都是仓库根。
  // pnpm 布局下根 node_modules 只有根直接依赖，@deepseek-ai/dsh-mcp-client 的
  // workspace 链接要锚到真正依赖它的 apps/cli。
  try {
    return createRequire(join(process.cwd(), "apps", "cli", "package.json")).resolve(name)
  } catch {}
  try {
    return createRequire(join(process.cwd(), "package.json")).resolve(name)
  } catch {}

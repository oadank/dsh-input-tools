...
export function applyRetroDelete(ctx) {
  ctx.effect(() => {
    ...
    if (typeof ctx.webServer?.register === 'function') {
      disposers.push(ctx.webServer.register({
        ...
        handler: async (req, res) => {
          ...
          // [本地改造 2026-09-25 老大抓到「排队没发出去的消息弹出了删回复」] 原实现这行用 agent?.inbox，
          // 而 const agent = liveAgent(sessionId) 声明在下面几十行处 —— 同一块作用域里 const 先用后声明
          // 是暂时性死区，一取用就抛 ReferenceError；前端 .catch(()=>({})) 把异常吞成空档位，弹窗于是掉进
          // 「状态未知」，而那一档带着「同时删除它的回复」勾选。node --check 只查语法查不出这个，混到今天。
          if (sessionId !== undefined && url.searchParams.get('pending') === '1') {
            const qtext = decodeURIComponent(String(url.searchParams.get('text') ?? ''))
            let items = []
            try { items = queueItemsOf(liveAgent(sessionId)?.inbox) } catch { items = [] }
    ...
    if (typeof ctx.webServer?.register === 'function') {
      // [探针 2026-09-25] 前端把界面行的真实身份原样报一条到宿主日志：一次看清 key 到底长啥样，别再靠猜改方向。
      disposers.push(ctx.webServer.register({
        kind: 'prefix',
        path: '/retro-dom',
        handler: async (req, res) => {
          ...
          try {
            const body = await readBody(req)
            log('dom ' + JSON.stringify(body?.dump ?? []).slice(0, 900))
            send(res, 200, { ok: true })
          } catch (e) { send(res, 400, { ok: false, error: String(e?.message ?? e) }) }
        },
      }))
      disposers.push(ctx.webServer.register({
        kind: 'prefix',
        ...
        handler: async (req, res) => {
          ...
          if (sessionId === undefined) { send(res, 400, { ok: false, error: 'sessionId 非法' }); return }
          const session = liveSession(sessionId)
          if (session === undefined) { send(res, 200, { ok: true, seqs: [], ids: [], turns: [], pairs: [], open: false }); return }
          const seqs = deletedSeqsOf(session)
          // 前端拿消息 id 寻址的行，只有靠 id 才知道该收哪一行（seq 未必对得上）。
          const ids = []
          const turns = []
          const texts = []
          // [本地改造 2026-09-25 老大批准 B 方案：跨设备根治] 前面几轮把「按 id 收行」和「按原文收行」
          // 分开做都不成：界面行 key 尾巴是输入框提交 id（与日志消息 id 两套 namespace，实测交集 0），
          // 光给原文又配不上轮号。这里一次给全 —— 「轮号|原文」成对返回，前端拿界面上的
          // data-chat-turn + 该行 innerText 直接对，任何设备刷新都收得住，不再依赖本机 localStorage。
          const umap = new Map(userMessagesOf(session).map((m) => [m.seq, m]))
          const pairs = []
          for (const s of seqs) {
            ...
            // ①′ [本地改造 2026-09-25 老大第四次报「删了还显示」] 身份这套已经靠不住（界面行的 key 是
            // 客户端自己拼的，kind:turn:step 还是消息 id 都不一定），所以把「这条消息的原文」也带上，
            // 前端按内容收行 —— 你眼睛看到什么就按什么收，不再赌门牌号。
            if (ev?.type === 'user/message') {
              const raw = flatTextOf(ev.data).replace(/\s+/g, '')
              if (raw !== '' && !texts.includes(raw.slice(0, 400))) texts.push(raw.slice(0, 400))
              const um = umap.get(s)
              if (um !== undefined && Number.isSafeInteger(um.turn)) {
                const p = um.turn + "|" + raw.slice(0, 80)
          ...
          send(res, 200, { ok: true, seqs, ids, turns, texts, pairs, open: true })
        },
      }))
    }

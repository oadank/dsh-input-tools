    ...
    if (typeof ctx.webServer?.register === 'function') {
      disposers.push(ctx.webServer.register({
        ...
        handler: async (req, res) => {
          ...
          // [本地改造 2026-09-24 老大实测] 先核内容再报档位：序号对不上这行的文字就是前端认错了行，
          // 直接报 mismatch，绝不继续往下编故事。
          ...
          if (!nodes.includes(seq)) {
            if (deletedSeqsOf(session).includes(seq)) { send(res, 200, { ok: true, stage: 'retracted' }); return }
            // 不在上下文里的三种情况分开报（老大实测质疑后的更正，别再拿"折叠"糊弄）：
            // 1) 这个序号根本不是能进上下文的消息 → not-message（前端拿错编号时也走这档）
            const event = session.eventAt?.(seq)
            if (event === undefined || !SURFACE_TYPES.includes(event.type)) {
              send(res, 200, { ok: true, stage: 'not-message' })
              return
            }
            // 2) 真被摘要顶掉 → compacted；3) 只是超出这次携带的范围 → windowed
            const watermark = compactionWatermark(session)
            send(res, 200, { ok: true, stage: watermark >= 0 && seq < watermark ? 'compacted' : 'windowed' })
            return
          }

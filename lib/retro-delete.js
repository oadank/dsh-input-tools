...
function resolveTargetSeq(session, addr) {
  if (addr.seq !== undefined) return addr.seq
  const list = userMessagesOf(session)
  if (addr.mid !== undefined && addr.mid !== '') {
    const hit = list.find((m) => m.id === addr.mid)
    if (hit !== undefined) return hit.seq
  }
  if (addr.turn !== undefined) {
    const cands = list.filter((m) => m.turn === addr.turn && textAgrees(addr.text ?? '', m.text))
    const k = Number.isSafeInteger(addr.occurrence) && addr.occurrence >= 0 ? addr.occurrence : 0
    if (cands.length > k) return cands[k].seq
  }
  // 最后一条兜底：整段会话里这行文字只有一条用户消息对得上 → 就认它；多条对得上则不猜（交给上层拒删）。
  if (addr.text !== undefined && addr.text !== '') {
    const byText = list.filter((m) => textAgrees(addr.text, m.text))
        ...
        handler: async (req, res) => {
          ...
          try {
            ...
            if (targets.length > 0) {
              ...
              send(res, 200, { ok: true, mode: 'tombstone', seq, voided: targets, stopped })
              return
            }

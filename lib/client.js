...
// 事件原生渲染）；rc.7 上 AI 语音音频已生成但界面不显示，属 rc.7 硬伤，引导用户使用源码版。
    ...
    function ToolbarLeft({ connection, sessionId }) {
      ...
      useEffect(() => {
        const iv = window.setInterval(() => {
          ...
          if (!ed) return;
          const txt = String(ed.innerText || "").replace(/[\u200b\u2060]/g, "").trim();
          ...
          if (txt === "") {
            // 框空了 = 优化稿被发出（或用户清空）：回填 sent 后归零，绝不把陈旧原文灌回空框
            if (!pair.sentReported) { pair.sentReported = true; reportOptFeedback(pair.id, { sent: true }); }
          ...
          // 发出前被手改（含点了还原原文）：回填 edited，只报一次；按钮语义不变
      ...
      return h("div", { ref: rootRef, "data-composer-left": "", style: { position: "relative", display: "inline-flex", alignItems: "center", gap: "12px" } },
        ...
        (() => {
          ...
          const onOptimize = async () => {
            ...
            const cur = getEditorText(ed).trim();
            const pair = optPairRef.current;
            if (optState === "done" && pair) {
              if (cur === "") {
                // 已发送（框空）：回填 sent 后归零，绝不把陈旧原文灌回空输入框
                if (!pair.sentReported) { pair.sentReported = true; reportOptFeedback(pair.id, { sent: true }); }
                optPairRef.current = null;
                setOptState("idle");
                setOptMeta(null);
              ...
              void setEditorText(ed, pair.original); // done 态：点击即还原（不比对文本，手改过也给回原文）
            ...
            try {
              ...
              const r = await fetch("/optimize-prompt", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ text: cur, conversationContext: convCtx }),
              });
              ...
              if (j && j.ok && j.optimized && j.fellBack !== true && String(j.optimized).trim() === cur) {
                ...
                optPairRef.current = { original: cur, optimized: j.optimized, id: j.id, drift: j.drift === true };
                ...
                // [2026-09-21 老大要求] 输入框上方透明元信息条（字数/耗时/检索/漂移…）
                setOptMeta(fmtOptMeta(j, cur.length, String(j.optimized).trim().length));
                ...
                setOptState("done");
                showOptToast("已改写：原 " + cur.length + " 字 → 新 " + String(j.optimized).trim().length + " 字（" + (deltaLen >= 0 ? "+" : "") + deltaLen + "）。点按钮可还原原文。", "ok");
                ...
                console.warn("[dsh-optimize] 硬事实丢失，已按原文返回: " + JSON.stringify(j.driftReport || {}));
                setOptState("drift");
                showOptToast("改写稿丢了关键信息" + (lost ? "（" + lost + "）" : "") + "，已按原文返回，避免改坏。可在设置页关掉 AI 漂移判定。", "warn");
          ...
          const composerEl = (typeof document !== "undefined") ? document.querySelector("[data-composer-input]") : null;
          const cRect = composerEl && composerEl.getBoundingClientRect ? composerEl.getBoundingClientRect() : null;
          const TOAST_STYLE = {
            position: "fixed", zIndex: 200,
            left: (cRect ? Math.max(8, cRect.left) : 16) + "px",
            bottom: (cRect && typeof window !== "undefined" ? Math.max(8, window.innerHeight - cRect.top + 8) : 96) + "px",
            maxWidth: "min(560px, 72vw)", width: "max-content",
            padding: "8px 10px", borderRadius: "8px", fontSize: "12px", lineHeight: 1.6,
            background: "rgba(24,28,36,.88)", color: "#e6e9ef",
            border: "1px solid rgba(255,255,255,.16)",
            backdropFilter: "blur(6px)", WebkitBackdropFilter: "blur(6px)",
            boxShadow: "0 6px 20px rgba(0,0,0,.35)", whiteSpace: "normal", textAlign: "left",
          };
          ...
          return h("span", { key: "optwrap", style: { position: "relative", display: "inline-flex", alignItems: "center" } },
            h("button", {
              ...
              "aria-label": optState === "idle" ? "优化提示词" : "提示词优化状态：" + optState,
              title: optState === "busy" ? "正在优化…" : optState === "done" ? "点击还原为原文" : optState === "copy" ? "输入框不接受程序化写入：优化稿已复制到剪贴板，在输入框里 Ctrl+V 覆盖即可" : optState === "same" ? "原文已足够明确，无需润色（这类消息不改写）" : optState === "drift" ? "改写稿丢了关键信息（数字/占位符/禁令），已按原文返回" : optState === "fail" ? "优化失败（详情见控制台，或看悬浮提示）" : "精炼输入框提示词",
              style: {
                ...circleBtn,
              ...
              ...circleBtnHover(false),
              onMouseDown: (e) => e.preventDefault(),
              onClick: () => { void onOptimize(); },
            }, optLabel),
            // [2026-09-21] 元信息条：与 toast 合并成一条、贴输入框上方、跟输入框同宽；
            // 过窄会把「重试 1 次」截成「重试…」；分开叠放会互相挡住。
            (optMeta || optToast) ? h("div", {
              style: {
                ...TOAST_STYLE,
                width: cRect ? Math.max(280, cRect.width) + "px" : "min(720px, 90vw)",
                maxWidth: "92vw",
              ...
              optMeta ? h("div", null, optMeta) : null,
              optToast ? h("div", {
                style: { marginTop: optMeta ? "4px" : "0", color: TOAST_ACCENT[optToast.kind] || "#c9d0d8" },
              }, optToast.text) : null,
            ) : null,
          );
        })(),
    ...
    function PromptOptimizeSection() {
      ...
      const TOGGLES = [
        ...
        ["useOpenmem", "读 openmem", "优化前读画像和相关记忆。关掉=更快。画像缓存 24h。"],
        ["retrievalRewrite", "检索先改写", "口语先改成 2~3 条好搜的短句再去查记忆，命中的更准。"],
    ...
    function VoiceSettingsSection() {
      ...
      // [2026-09-19] 配置编辑锁：输入框/下拉默认灰色锁定，点「🔒 修改配置」才解锁。
      // 背景：克隆列表、local url 两次被"旧快照整份写回"抹掉——锁定编辑入口防手滑误改。
      const [editMode, setEditModeState] = useState(false);
      const editModeRef = useRef(false);
      const setEditMode = (v) => { editModeRef.current = v; setEditModeState(v); };
      ...
      const setEngine = (key, patch, autoSave, topPatch) => {
        if (!editModeRef.current) return; // [2026-09-19] 锁定态拦截：一切配置写入必须先点「修改配置」解锁
        setConfig((c) => {
          if (c === null) return c;
      ...
      const voiceSelect = (engine, current, voices, onChange, showPreview) => h("div", { style: { display: "flex", gap: "6px", alignItems: "center" } },
        h("select", { disabled: !editMode, value: current, onChange: (e) => onChange(e.target.value), style: vInput },
      ...
      const vdLockRow = (label, keyName, value, onValue, optionsOrPlaceholder) => h("div", { style: { display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap", fontSize: "12px", color: "var(--dsw-alias-label-secondary,#9aa3ad)" } },
        h("label", { style: { display: "inline-flex", alignItems: "center", gap: "5px", cursor: "pointer" } },
          h("input", { disabled: !editMode, type: "checkbox", checked: eng.voicedesign?.[keyName] === true, onChange: (e) => setEngine("voicedesign", { [keyName]: e.target.checked }, true, VD_TOP), style: { accentColor: "var(--vk-accent,#4b6fff)", cursor: "pointer", width: "13px", height: "13px" } }),
          label),
        Array.isArray(optionsOrPlaceholder) && eng.voicedesign?.[keyName] === true ? h("select", { disabled: !editMode,
          value: value,
        ...
        typeof optionsOrPlaceholder === "string" && eng.voicedesign?.[keyName] === true ? h("input", { disabled: !editMode,
      ...
      return h("div", { className: "vk-voice-panel", style: { display: "flex", flexDirection: "column", gap: "14px", padding: "16px", width: "100%", boxSizing: "border-box" } },
        ...
        // [2026-09-19] 配置编辑锁开关：默认锁定，点「修改配置」解锁（配置字段全部灰显只读）
        h("div", { style: { display: "flex", justifyContent: "flex-end" } },
          h("button", {
            type: "button",
            onClick: () => setEditMode(!editMode),
            style: {
              background: editMode ? "var(--vk-accent,#4b6fff)" : "transparent",
              color: editMode ? "#fff" : "var(--dsw-alias-label-secondary,#9aa3ad)",
              border: editMode ? "none" : "1px solid var(--dsw-alias-border-l1,#333a45)",
              borderRadius: "999px", padding: "6px 14px", fontSize: "12.5px", fontWeight: 600, cursor: "pointer", flex: "none",
            },
          }, editMode ? "🔓 完成修改" : "🔒 修改配置"),
        ),
        ...
        h("div", { style: { border: "1px solid var(--dsw-alias-border-l1,#333a45)", borderRadius: "10px", padding: "10px 12px", display: "flex", flexDirection: "column", gap: "8px", background: "rgba(128,128,128,.05)" } },
          ...
          h("div", { style: { display: "flex", flexDirection: "column", gap: "10px" } },
            h("div", { style: { display: "flex", flexWrap: "wrap", gap: "10px" } },
              vField("模式", h("select", { disabled: !editMode,
              ...
              (eng.asr.mode ?? "service") === "service" ? vField("本地服务地址", h("input", { disabled: !editMode, value: eng.asr.url ?? "", onChange: (e) => setEngine("asr", { url: e.target.value }, true), placeholder: "http://127.0.0.1:18790", style: vInput })) : null,
              (eng.asr.mode ?? "service") === "cmd" ? vField("本地命令", h("input", { disabled: !editMode, value: eng.asr.cmd ?? "", onChange: (e) => setEngine("asr", { cmd: e.target.value }, true), placeholder: "sherpa-onnx-offline.exe --tokens=... --sense-voice-model=... --num-threads=4", style: vInput })) : null,
              (eng.asr.mode ?? "service") === "api" ? [
                secretField("API Key", "asr", eng.asr.apiKey ?? "", (e) => setEngine("asr", { apiKey: e.target.value }, true), "sk-..."),
                vField("API 地址", h("input", { disabled: !editMode, value: eng.asr.apiBaseUrl ?? "", onChange: (e) => setEngine("asr", { apiBaseUrl: e.target.value }, true), placeholder: "https://api.openai.com/v1", style: vInput })),
              ] : null,
            ),
        ...
        // 默认引擎
        vField("默认语音引擎", h("select", { disabled: !editMode,
          ...
          () => h("div", { style: { display: "flex", flexDirection: "column", gap: "10px" } },
              ...
              vdGroup === "examples" ? h("div", { style: { display: "flex", flexDirection: "column", gap: "6px" } },
                VOICE_DESIGN_EXAMPLES.map((ex, i) => {
                  ...
                  return h("div", {
                    ...
                    h("input", { disabled: !editMode, type: "radio", name: "vd-mode", checked: active, onChange: () => pickVdMode(key), style: { accentColor: "var(--vk-accent,#4b6fff)", cursor: "pointer", flex: "none", width: "14px", height: "14px", margin: "0" } }),
            ...
            h("div", { style: { display: "flex", flexDirection: "column", gap: "6px", borderTop: "1px dashed var(--dsw-alias-border-l1,#333a45)", borderLeft: "2px solid #f472b6", paddingTop: "8px", paddingLeft: "8px" } },
              ...
              h("div", { style: { display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap", borderTop: "1px dashed var(--dsw-alias-border-l1,#333a45)", paddingTop: "8px" } },
                ...
                h("input", { disabled: !editMode, ref: cloneFileRef, type: "file", accept: ".mp3,.wav,audio/mpeg,audio/wav", style: { display: "none" }, onChange: (e) => { const f = e.target.files && e.target.files[0]; if (f !== undefined && f !== null) void addCloneSample(f); } }),
        ...
        vCard(h("span", { style: { display: "inline-flex", alignItems: "center", gap: "6px", flexWrap: "wrap" } },
          ...
          [
            ...
            h("div", { style: { display: "flex", flexWrap: "wrap", gap: "10px" } },
              vField("常驻服务地址", h("input", { disabled: !editMode, value: eng.audio8?.url ?? "http://127.0.0.1:18795", onChange: (e) => setEngine("audio8", { url: e.target.value }, true), placeholder: "http://127.0.0.1:18795", style: vInput })),
            ),
            ...
            h("div", { style: { display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap", borderTop: "1px dashed var(--dsw-alias-border-l1,#333a45)", paddingTop: "8px" } },
              ...
              h("input", { disabled: !editMode, ref: a8FileRef, type: "file", accept: "audio/*", style: { display: "none" }, onChange: (e) => { const f = e.target.files && e.target.files[0]; if (f !== undefined && f !== null) void addAudio8Voice(f); } }),
        ...
        vCard(h("span", { style: { display: "inline-flex", alignItems: "center", gap: "6px", flexWrap: "wrap" } },
          ...
          [
            h("div", { style: { display: "flex", flexWrap: "wrap", gap: "10px" } },
              vField("本地命令（每次调用启动进程）", h("input", { disabled: !editMode, value: eng.local.cmd ?? "", onChange: (e) => setEngine("local", { cmd: e.target.value }, true), placeholder: "如 node <插件目录>\\local-tts.mjs（安装脚本会自动填好）", style: vInput })),
              vField("HTTP 服务地址（常驻模式）", h("input", { disabled: !editMode, value: eng.local.url ?? "", onChange: (e) => setEngine("local", { url: e.target.value }, true), placeholder: "如 http://127.0.0.1:5000/tts（POST {text} 返回音频）", style: vInput })),
            ),
            h("div", { style: { display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap" } },

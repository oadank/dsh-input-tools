...
// 事件原生渲染）；rc.7 上 AI 语音音频已生成但界面不显示，属 rc.7 硬伤，引导用户使用源码版。
    ...
    function VoiceSettingsSection() {
      ...
      // [2026-08-22] 图片识别（look_image）「?」提示 state
      const [visionTipPinned, setVisionTipPinned] = useState(false);
      const [visionTipHover, setVisionTipHover] = useState(false);
      // [2026-08-22] 图片识别配置测试：模式 + 结果 + 放大查看弹窗
      ...
      // [2026-08-22] 工具提示词编辑：key=describe|text|reverse|null
      const [visionEditKey, setVisionEditKey] = useState(null);
      ...
      // [2026-08-22] 各模式介绍（随测试模式下拉切换的「?」显示）
      const VISION_MODE_INTRO = {
        describe: "看图描述：AI 用简短中文描述图片内容（简洁一两句）。点 ✎ 可查看/编辑实际使用的提示词。",
        reverse: "像素级反推：把图反推成可直接用于 AI 生图（即梦/可灵/SD/Midjourney 等）的完整中文提示词。默认使用专业反推提示词，输出较长。点 ✎ 可查看/编辑。",
        text: "提取文字：逐字提取图中所有文字，按画面位置分行输出。点 ✎ 可查看/编辑。",
      };
      const openPromptEditor = (key) => setVisionEditKey(key);
      // 当前某工具的有效提示词（配置值优先，空=内置默认；reverse 内置=反推提示词文件）
      const effectivePrompt = (key) => {
        const cfg = (config?.vision?.prompts ?? {})[key];
        if (typeof cfg === "string" && cfg.trim() !== "") return cfg;
        return key === "reverse" ? "（默认：visionqa-reverse-prompt.txt 反推提示词，编辑后即覆盖）" : "（留空 = 使用内置默认提示词，可编辑自定义）";
      };
      // 编辑提示词实时保存到配置；恢复默认=置空该 key（回退内置/文件）
      ...
      // [2026-08-22] 图片识别配置（顶层 vision 段，不在 engines 里）：读写同 /voice-config
      ...
      // [2026-08-22] 图片识别配置测试：调 host /voice-config/vision-test（内置测试图 + 所选模式）
      const testVision = () => {
      ...
      return h("div", { style: { display: "flex", flexDirection: "column", gap: "14px", padding: "16px", width: "100%", boxSizing: "border-box" } },
        ...
        // ⑥ 图片识别（look_image 工具，[2026-08-22] 自研，替代 modlens/外部识图 MCP）
        h("div", { style: { border: "1px solid var(--dsw-alias-border-l1,#333a45)", borderRadius: "10px", padding: "10px 12px", display: "flex", flexDirection: "column", gap: "8px", background: "rgba(128,128,128,.05)" } },
          h("div", { style: { display: "flex", alignItems: "center", gap: "8px" } },
            h("span", { style: { display: "inline-flex", width: "22px", height: "22px", borderRadius: "6px", background: "rgba(128,128,128,.12)", alignItems: "center", justifyContent: "center", color: "var(--vk-accent,#4b6fff)", flex: "none", fontSize: "12px" } }, "🖼️"),
            h("span", { style: { fontSize: "13px", fontWeight: 600, color: "var(--dsw-alias-label-primary,#e6e9ef)" } }, "图片识别"),
            helpTip(
              "模型收到图片后，AI 会自动调用 look_image 工具识别。三种模式：\n"
                + "· describe（默认）= 详细看图描述；\n"
                + "· reverse = 像素级反推——把图反推成可直接用于 AI 生图（即梦/可灵/SD/Midjourney 等）的完整中文提示词；\n"
                + "· text = 逐字提取图中文字。\n"
                + "后端可配本地 ollama（默认）或任意 OpenAI 兼容服务（vllm 等）：填地址和模型名即可，在线服务再加 API Key。"
                + "不依赖任何外部 MCP / 第三方识图插件。",
              visionTipPinned, setVisionTipPinned, visionTipHover, setVisionTipHover, "", "top"),
          ),
          h("div", { style: { display: "flex", flexDirection: "column", gap: "10px" } },
            h("div", { style: { display: "flex", flexWrap: "wrap", gap: "10px" } },
              vField("部署位置", h("select", {
                value: ["online", "openai"].includes(config.vision?.provider ?? "local") ? "online" : "local",
                onChange: (e) => {
                  const p = e.target.value;
                  // [2026-08-22] 切换自动填默认地址（本地→ollama，在线→硅基流动示例）
                ...
                h("option", { value: "local" }, "本地（ollama / sglang / vllm 等）"),
                h("option", { value: "online" }, "在线（云端 API）"))),
              vField("API 地址（填到 /v1）", h("input", {
                value: config.vision?.baseUrl ?? "",
                onChange: (e) => setVision({ baseUrl: e.target.value }, true),
                placeholder: ["online", "openai"].includes(config.vision?.provider ?? "local") ? "https://api.siliconflow.cn/v1" : "http://127.0.0.1:11434/v1",
                style: vInput,
              })),
              vField("模型", h("input", {
              ...
              ["online", "openai"].includes(config.vision?.provider ?? "local")
                ? secretField("API Key（在线服务必填）", "vision", config.vision?.apiKey ?? "", (e) => setVision({ apiKey: e.target.value }, true), "sk-...")
                : null,
            ),
            h("div", { style: { fontSize: "12px", opacity: .75, lineHeight: 1.6 } },
              "看图工具 look_image：describe=看图描述 / reverse=像素级反推生图提示词 / text=提取文字。AI 按你提问自动选模式。"),
            // [2026-08-22] 识图配置测试：测试图缩略图(点击放大) + 选模式(带?介绍与✎编辑) + 测试按钮
            h("div", { style: { display: "flex", alignItems: "center", gap: "12px", flexWrap: "wrap", borderTop: "1px solid var(--dsw-alias-border-l1,#333a45)", paddingTop: "10px" } },
              ...
              h("div", { style: { display: "flex", flexDirection: "column", gap: "6px", flex: "1", minWidth: "260px" } },
                h("div", { style: { display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap" } },
                  ...
                  // [2026-08-22] 「?」随模式切换显示该模式介绍
                  helpTip(VISION_MODE_INTRO[visionTestTask] ?? VISION_MODE_INTRO.describe,
                    visionModeTipPinned, setVisionModeTipPinned, visionModeTipHover, setVisionModeTipHover, "", "top"),
                  // [2026-08-22] 「✎」编辑当前模式的提示词
                  h("button", {
                    type: "button",
                    ...
                    style: {
                      ...
                      color: "var(--dsw-alias-link,#5b9cff)", fontSize: "15px", lineHeight: "1",
                ...
                h("span", { style: { fontSize: "11px", opacity: .7 } }, "内置测试图（含文字，可验证描述/反推/提取），点击缩略图放大，识图结果与图对照即知对错"),
              ),
            ),
            // 测试结果
            visionTestResult !== null ? h("div", { style: {
              ...
              h("div", { style: { display: "flex", alignItems: "center", gap: "8px", marginBottom: "4px", fontSize: "11px", opacity: .8 } },
        ...
        // [2026-08-22] 放大查看提示词弹窗（可编辑/复制/关闭）
        ...
        // [2026-08-22] 工具提示词编辑弹窗（describe/reverse/text 各自可编辑 + 恢复默认）
        visionEditKey !== null ? h("div", {
          ...
          h("div", { style: {
            ...
            h("div", { style: { display: "flex", alignItems: "center", gap: "8px", padding: "10px 14px", borderBottom: "1px solid var(--dsw-alias-border-l1,#333a45)", fontSize: "13px", fontWeight: 600 } },
              "✎ 编辑 " + (VISION_TASK_LABELS[visionEditKey] ?? visionEditKey) + " 提示词",
              h("span", { style: { marginLeft: "auto", display: "flex", gap: "6px" } },
                h("button", {
                  type: "button",
                  style: btnSmall,
                  onMouseDown: (e) => e.preventDefault(),
                  onClick: () => copyText((config?.vision?.prompts ?? {})[visionEditKey] ?? ""),
                }, "复制"),
                h("button", {
                  type: "button",
                  style: { ...btnSmall, background: "rgba(229,164,58,.2)", color: "#e5a53a" },
                  onMouseDown: (e) => e.preventDefault(),
            ...
            h("textarea", {
              value: (config?.vision?.prompts ?? {})[visionEditKey] ?? "",
              onChange: (e) => savePromptEdit(visionEditKey, e.target.value),
              placeholder: visionEditKey === "reverse"
                ? "留空 = 使用默认反推提示词（visionqa-reverse-prompt.txt，专业像素级反推）"
                : "留空 = 使用内置默认提示词；此处输入内容将覆盖默认",
              spellCheck: false,
              style: {
                flex: "1", minHeight: "320px", margin: "12px 14px", padding: "10px 12px",
                background: "rgba(128,128,128,.06)", color: "inherit", border: "1px solid var(--dsw-alias-border-l1,#333a45)",
            ...
            h("div", { style: { padding: "0 14px 10px", fontSize: "11px", opacity: .7 } },
              "编辑实时保存，保存后 look_image 用此提示词识图；「恢复默认」清除自定义回到内置。"),
          ),
        ) : null,
        // [2026-08-22] 测试图点击放大（大图 modal，仿聊天图片放大）
        visionImgZoom ? h("div", {
          style: {
          ...
          h("div", { style: { position: "relative", maxWidth: "92vw", maxHeight: "88vh" } },
            h("img", {
              src: "/voice-config/vision-test-image",
              alt: "测试图大图",
              style: { maxWidth: "92vw", maxHeight: "88vh", objectFit: "contain", borderRadius: "10px", boxShadow: "0 12px 48px rgba(0,0,0,.6)", display: "block", background: "rgba(255,255,255,.04)" },
            }),
            h("button", {
              type: "button",
              title: "关闭",

          ...
          h("div", { style: { display: "flex", alignItems: "center", gap: "8px" } },
            ...
            helpTip(
              "模型收到图片后，AI 会自动调用 look_image 工具识别。三种模式：\n"
                ...
                + "不依赖任何外部 MCP / 第三方识图插件。",
              visionTipPinned, setVisionTipPinned, visionTipHover, setVisionTipHover, "center"),
          ),
          h("div", { style: { display: "flex", flexDirection: "column", gap: "10px" } },
            h("div", { style: { display: "flex", flexWrap: "wrap", gap: "10px" } },
              vField("后端类型", h("select", {
                value: config.vision?.provider ?? "ollama",
                onChange: (e) => setVision({ provider: e.target.value }, true),
                style: vInput,
              },
                h("option", { value: "ollama" }, "本地 ollama"),
                h("option", { value: "openai" }, "OpenAI 兼容（vllm 等）"))),
              vField((config.vision?.provider ?? "ollama") === "openai" ? "API 地址" : "ollama 地址", h("input", {
                value: config.vision?.baseUrl ?? "",
                onChange: (e) => setVision({ baseUrl: e.target.value }, true),
                placeholder: (config.vision?.provider ?? "ollama") === "openai" ? "http://127.0.0.1:8000/v1" : "http://127.0.0.1:11434",
                style: vInput,
              })),
              vField("模型", h("input", {

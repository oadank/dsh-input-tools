      ...
      return h("div", { style: { display: "flex", flexDirection: "column", gap: "14px", padding: "16px", width: "100%", boxSizing: "border-box" } },
        ...
        vField("默认语音引擎", h("select", {
          ...
          onChange: (e) => {
            ...
            // [本地改造 2026-08-22] 选「语音设计」时若还没填音色描述，默认用「纪录片旁白」指令
            if (e.target.value === "voicedesign" && (config.engines?.voicedesign?.context ?? "").trim() === "") {
              next.engines = { ...(config.engines ?? {}), voicedesign: { ...(config.engines?.voicedesign ?? {}), context: VOICE_DESIGN_EXAMPLES[1].instruct } };
            }
            setConfig(next);
            if (saveTimerRef.current !== null) window.clearTimeout(saveTimerRef.current);
            saveTimerRef.current = window.setTimeout(() => {
        ...
        vCard(h("span", { style: { display: "inline-flex", alignItems: "center", gap: "6px", flexWrap: "wrap" } },
          ...
          () => h("div", { style: { display: "flex", flexDirection: "column", gap: "10px" } },
            ...
            // [2026-08-22] 语音设计：MiMo-V2.5-TTS-VoiceDesign（官方示例 + 使用此指令 + 自定义指令，始终显示）
            h("div", { style: { display: "flex", flexDirection: "column", gap: "8px", borderTop: "1px dashed var(--dsw-alias-border-l1,#333a45)", paddingTop: "8px" } },
              h("div", { style: { display: "flex", alignItems: "center", gap: "6px" } },
                h("span", { style: { fontSize: "12.5px", fontWeight: 600, color: "var(--dsw-alias-label-primary,#e6e9ef)" } }, "语音设计：MiMo-V2.5-TTS-VoiceDesign"),
                helpTip("「音色设计 VoiceDesign」用一段文字描述你想要的声音（性别/年龄/质感/语速/情绪），AI 照着念。下面 3 个官方示例可直接点「使用此指令」套用，或在下方自定义框里自己写一段音色描述；选为默认语音引擎后默认用「纪录片旁白」指令。勾选「AI 自动情绪」后，AI 还会根据对话情境自动编写音色描述（任务成功兴奋道喜、你生气委屈道歉、你难过温柔安慰）——该能力尚未充分测试。", designTipPinned, setDesignTipPinned, designTipHover, setDesignTipHover, "center", "top"),
              ),
              h("div", { style: { fontSize: "12px", color: "var(--dsw-alias-label-secondary,#9aa3ad)" } }, "官方示例（点播放试听；点「使用此指令」套用其音色描述）"),
              VOICE_DESIGN_EXAMPLES.map((ex, i) => {
                const active = (eng.voicedesign.context ?? "") === ex.instruct;
                return h("div", { key: ex.title, style: { border: "1px solid var(--dsw-alias-border-l1,#333a45)", borderRadius: "8px", padding: "8px 10px", display: "flex", flexDirection: "column", gap: "6px" } },
                  h("div", { style: { display: "flex", alignItems: "center", gap: "8px" } },
                    ...
                    h("button", {
                      type: "button",
                      onClick: () => setEngine("voicedesign", { context: ex.instruct }, true),
                      style: {
                        marginLeft: "auto", flex: "none", border: "none", borderRadius: "999px", padding: "4px 12px",
                        fontSize: "11.5px", fontWeight: 600, cursor: "pointer",
                        background: active ? "var(--vk-accent,#4b6fff)" : "rgba(128,128,128,.15)",
              ...
              // [2026-08-22] 自定义音色描述 + AI 自动情绪开关
              h("div", { style: { display: "flex", flexDirection: "column", gap: "6px", borderTop: "1px dashed var(--dsw-alias-border-l1,#333a45)", paddingTop: "8px" } },
                h("div", { style: { display: "flex", alignItems: "center", gap: "8px" } },
                  h("span", { style: { fontSize: "12.5px", fontWeight: 600, color: "var(--dsw-alias-label-primary,#e6e9ef)" } }, "自定义音色描述"),
                ...
                h("label", { style: { display: "flex", alignItems: "center", gap: "6px", fontSize: "12px", color: "var(--dsw-alias-label-secondary,#9aa3ad)", cursor: "pointer" } },
                  h("input", { type: "checkbox", checked: eng.voicedesign.emotion === true, onChange: (e) => setEngine("voicedesign", { emotion: e.target.checked }, true), style: { accentColor: "var(--vk-accent,#4b6fff)" } }),
                  "AI 自动情绪：AI 根据对话情境自动写音色描述（任务成功兴奋道喜 / 生气委屈道歉 / 难过温柔安慰）——尚未充分测试，不好用就关掉"),
              ),
            ),

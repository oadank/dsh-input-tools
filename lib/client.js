    ...
    function ToolbarLeft({ connection, sessionId }) {
      ...
      useEffect(() => {
        const iv = window.setInterval(() => {
          ...
          if (!optPairRef.current) {
            try {
              const raw = window.sessionStorage.getItem(OPT_PAIR_KEY);
              if (raw === null) return;
              ...
              if (p && p.optimized && txt !== "" && txt === String(p.optimized).trim()) {
                ...
                window.sessionStorage.removeItem(OPT_PAIR_KEY); // 草稿已清空：作废
              }
            } catch { try { window.sessionStorage.removeItem(OPT_PAIR_KEY); } catch { /* 无存储权限 */ } }
            return;
          }
          if (txt === "") {
      ...
      return h("div", { ref: rootRef, "data-composer-left": "", style: { position: "relative", display: "inline-flex", alignItems: "center", gap: "12px" } },
        ...
        (() => {
          ...
          const findEditor = () => {
            const lex = document.querySelector("[data-composer-input]");
            if (lex) return lex;
          ...
          const onOptimize = async () => {
            ...
            if (optState === "done" && pair) {
              if (cur === "") {
                // 已发送（框空）：归零，绝不把陈旧原文灌回空输入框
                optPairRef.current = null;
                setOptState("idle");
                try { window.sessionStorage.removeItem(OPT_PAIR_KEY); } catch { /* 无存储权限 */ }
                return;
              }
              setEditorText(ed, pair.original); // done 态：点击即还原（不比对文本，手改过也给回原文）
              optPairRef.current = null;
              setOptState("idle");
              try { window.sessionStorage.removeItem(OPT_PAIR_KEY); } catch { /* 无存储权限 */ }
              return;
            ...
            try {
              ...
              if (j && j.ok && j.optimized) {
                optPairRef.current = { original: cur, optimized: j.optimized };
                try { window.sessionStorage.setItem(OPT_PAIR_KEY, JSON.stringify(optPairRef.current)); } catch { /* 无存储权限 */ }
                setEditorText(ed, j.optimized);
                setOptState("done");
              } else {
                console.error("[dsh-optimize] 优化失败: " + ((j && j.error) || ("http " + r.status)));
                setOptState("fail");
          ...
          return h("button", {
            ...
            "aria-label": "优化提示词",
            title: optState === "busy" ? "优化中…" : optState === "done" ? "点击还原为原文" : optState === "fail" ? "优化失败（详情见控制台）" : "精炼输入框提示词",
            style: {
              ...circleBtn,
              ...(optState === "busy" ? { opacity: 0.7 } : {}),
              ...(optState === "done" ? { background: "#2f6feb", color: "#fff", width: "auto", padding: "0 10px" } : {}),
              ...(optState === "fail" ? { background: "#e5484d", color: "#fff" } : {}),
            },
            ...circleBtnHover(false),
            onMouseDown: (e) => e.preventDefault(),
            onClick: () => { void onOptimize(); },
          }, optState === "busy"
            ? h("span", { style: { fontSize: "15px", lineHeight: 1, display: "inline-block", animation: "dsh-opt-spin 0.9s linear infinite" }, "aria-hidden": "true" }, "🔄")
            : optState === "done"
              ? h("span", { style: { fontSize: "12px", lineHeight: 1, whiteSpace: "nowrap" } }, "↩ 还原原文")
              : optState === "fail"
                ? h("span", { style: { fontSize: "15px", lineHeight: 1, fontWeight: 700 }, "aria-hidden": "true" }, "!")
                : h("span", { style: { fontSize: "15px", lineHeight: 1 }, "aria-hidden": "true" }, "⚡"));
        })(),

...
// 事件原生渲染）；rc.7 上 AI 语音音频已生成但界面不显示，属 rc.7 硬伤，引导用户使用源码版。
    ...
    function ToolbarLeft({ connection, sessionId }) {
      ...
      // [2026-08-22] 把路径文本插入官方输入框（modlens insertText 技巧：execCommand 触发 React input 事件，失败用原型 setter）
      ...
      const removeFromComposer = (text) => {
        ...
        try {
          ...
          if (idx === -1) return;
          // [2026-08-22 修] 模拟用户删除（选中标记文本 → execCommand delete），与插入同路线，React 受控组件安全
          try {
            el.focus();
          ...
          const proto = window.HTMLTextAreaElement.prototype;
          const setter = Object.getOwnPropertyDescriptor(proto, "value").set;
          setter.call(el, v.slice(0, idx) + v.slice(idx + text.length));
          el.dispatchEvent(new Event("input", { bubbles: true }));
        } catch { /* 忽略 */ }
      };

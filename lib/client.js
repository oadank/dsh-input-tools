      ...
      return h("div", { style: { display: "flex", flexDirection: "column", gap: "14px", padding: "16px", width: "100%", boxSizing: "border-box" } },
          ...
          h("div", { style: { display: "flex", alignItems: "center", gap: "8px" } },
            ...
            helpTip(
              "模型收到图片后，AI 会自动调用 look_image 工具识别。三种模式：\n"
                ...
                + "不依赖任何外部 MCP / 第三方识图插件。",
              visionTipPinned, setVisionTipPinned, visionTipHover, setVisionTipHover, "right", "top"),
          ),
          h("div", { style: { display: "flex", flexDirection: "column", gap: "10px" } },
            h("div", { style: { display: "flex", flexWrap: "wrap", gap: "10px" } },
              vField("后端类型", h("select", {
                value: config.vision?.provider ?? "ollama",
                onChange: (e) => {
                  const p = e.target.value;
                  setVision({ provider: p }, false);
                  // [2026-08-22] 切换后端时自动填对应预设地址（用户可改）
                  if (p === "openai" && !String(config.vision?.baseUrl ?? "").includes("/v1")) {
                    setVision({ provider: p, baseUrl: config.vision?.baseUrl && config.vision.baseUrl !== "http://127.0.0.1:11434/v1" ? config.vision.baseUrl : "http://127.0.0.1:8000/v1" }, true);
                  } else if (p === "ollama" && !String(config.vision?.baseUrl ?? "").includes(":8000")) {
                    setVision({ provider: p, baseUrl: "http://127.0.0.1:11434/v1" }, true);
                  } else {
                    setVision({ provider: p }, true);
                  }
                },
                style: vInput,
              },
                h("option", { value: "ollama" }, "本地 ollama"),
                h("option", { value: "openai" }, "OpenAI 兼容（vllm 等）"))),
              vField("API 地址（填到 /v1）", h("input", {
                value: config.vision?.baseUrl ?? "",
                onChange: (e) => setVision({ baseUrl: e.target.value }, true),
                placeholder: (config.vision?.provider ?? "ollama") === "openai" ? "http://127.0.0.1:8000/v1" : "http://127.0.0.1:11434/v1",
                style: vInput,
              })),
              vField("模型", h("input", {
              ...
              (config.vision?.provider ?? "ollama") === "openai"
                ? secretField("API Key（在线服务填）", "vision", config.vision?.apiKey ?? "", (e) => setVision({ apiKey: e.target.value }, true), "sk-...")
                : null,
            ),
            ...
            // [2026-08-22] 识图配置测试：选模式 → 内置测试图跑一遍识图链路
            h("div", { style: { display: "flex", alignItems: "center", gap: "10px", flexWrap: "wrap", borderTop: "1px solid var(--dsw-alias-border-l1,#333a45)", paddingTop: "8px" } },
              ...
              h("span", { style: { fontSize: "11px", opacity: .7 } }, "用内置截图验证配置是否连通"),
            ),
            // 测试结果
        ...
        visionZoom !== null ? h("div", {
          ...
          h("div", { style: {
            ...
            h("textarea", {
              value: visionZoom,
              onChange: (e) => setVisionZoom(e.target.value),
              spellCheck: false,
              style: {

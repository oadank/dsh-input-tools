    ...
    function ToolbarLeft({ connection, sessionId }) {
      ...
      useEffect(() => {
        try {
          ...
          const mark = ` ⚡>${optState} txt=${tl} pair=${pl} eq=${same ? "Y" : "N"} « ${optTraceRef.current.slice(-3).join(" ")}`;
          document.title = String(document.title || "").replace(/ ⚡>[\s\S]*$/, "") + mark;
          ...
          const onOptimize = async () => {
            ...
            try {
              ...
              if (j && j.ok && j.optimized && j.fellBack !== true && normOpt(j.optimized) === normOpt(cur)) {
                ...
                setOptMeta(byGate
                  ? ("⛔ 闸门拦下：判为「不值得改写」的短句/寒暄 ｜ " + ((Date.now() - textT0) / 1000).toFixed(1) + "s（省掉一次改写）")
                  : fmtOptMeta(j, cur.length, cur.length, Date.now() - textT0));
                showOptToast(byGate
                  ? "这句被前置闸门判成「短到不用改写」，原文没动、也没花改写的钱。要强行走一遍：写到 40 字以上，或在设置里关掉「改写前置闸门」。"
                  : "改写模型回来的一字未改：原文已经够清楚了。", "info");

...
// 事件原生渲染）；rc.7 上 AI 语音音频已生成但界面不显示，属 rc.7 硬伤，引导用户使用源码版。
    ...
    function ToolbarLeft({ connection, sessionId }) {
      ...
      // [2026-09-21] ⚡ 提示词优化：busy 态 + {original, optimized} 成对暂存（再点还原）
      const optBusyRef = useRef(false);
      const [optBusyTick, setOptBusyTick] = useState(0);
      ...
      return h("div", { ref: rootRef, "data-composer-left": "", style: { position: "relative", display: "inline-flex", alignItems: "center", gap: "12px" } },
        ...
        (() => {
          ...
          const onOptimize = async () => {
            ...
            if (pair && cur === pair.optimized) {
              setNativeValue(ta, pair.original); // 再点 → 还原
              optPairRef.current = null;
              setOptBusyTick((n) => n + 1);
              return;
            }
            if (pair && cur !== pair.optimized) optPairRef.current = null; // 用户手改过，成对失效
            if (cur === "") return;
            optBusyRef.current = true;
            setOptBusyTick((n) => n + 1);
            try {
          ...
          return h("button", {
            ...
            "aria-label": "优化提示词",
            title: optBusyRef.current ? "优化中…" : (optPairRef.current ? "点击还原为原文" : "精炼输入框提示词（再点还原）"),
            style: {
              ...circleBtn,
              ...(optBusyRef.current ? { opacity: 0.5 } : {}),
              ...(optPairRef.current ? { background: "#2f6feb", color: "#fff" } : {}),
            },
            ...circleBtnHover(false),
            onMouseDown: (e) => e.preventDefault(),
            onClick: () => { void onOptimize(); },
          // eslint-disable-next-line
          }, h("span", { style: { fontSize: "15px", lineHeight: 1 }, "aria-hidden": "true" }, optBusyRef.current ? "…" : "⚡"));
        })(),

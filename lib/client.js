    ...
    function VoiceSettingsSection() {
      ...
      return h("div", { style: { display: "flex", flexDirection: "column", gap: "14px", padding: "16px", width: "100%", boxSizing: "border-box" } },
        ...
        vCard(h("span", { style: { display: "inline-flex", alignItems: "center", gap: "6px", flexWrap: "wrap" } },
          ...
          helpTip("Audio8 TTS：本机 CPU 零样本声音克隆（0.6B INT4）。先在服务器上用 register_voice.py 注册音色（录音 ≤30 秒 + 逐字文本），这里下拉选音色即可。不选则自动用最新注册的音色。CPU 推理较慢（一句约 15-30 秒），适合语音回复不赶时间的场景。", false, () => {}, false, () => {}, "center"),
        ), openCards.audio8, () => toggleCard("audio8"),
          [
            h("div", { style: { display: "flex", flexWrap: "wrap", gap: "10px" } },
              vField("克隆音色（已注册）", (() => {
                const voices = meta?.audio8Voices ?? [];
                return h("select", { value: eng.audio8?.voice ?? "", onChange: (e) => setEngine("audio8", { voice: e.target.value }, true), style: vInput },
                  h("option", { value: "" }, voices.length ? "自动（最新注册）" : "（暂无已注册音色）"),
                  voices.map((v) => h("option", { key: v, value: v }, v)),
                );
              })()),
              vField("命令行（一般不用改）", h("input", { value: eng.audio8?.cmd ?? "", onChange: (e) => setEngine("audio8", { cmd: e.target.value }, true), placeholder: "node C:\\D\\opt\\audio8-tts\\audio8-tts.mjs", style: vInput })),
            ),
            h("div", { style: { display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap" } },
              previewBtn("audio8-preview", "试听 Audio8", () => previewVoice("audio8", eng.audio8?.voice || undefined, undefined, undefined, "audio8-preview")),
              h("span", { style: { fontSize: "12px", color: "var(--dsw-alias-label-secondary,#9aa3ad)" } }, "CPU 合成约 15-30 秒，试听请稍等"),
            ),
          ]),

...
// 事件原生渲染）；rc.7 上 AI 语音音频已生成但界面不显示，属 rc.7 硬伤，引导用户使用源码版。
    ...
    function VoiceSettingsSection() {
      ...
      const a8Btn = (tag, text, onClick) => h("button", {
      ...
      return h("div", { style: { display: "flex", flexDirection: "column", gap: "14px", padding: "16px", width: "100%", boxSizing: "border-box" } },
        ...
        vCard(h("span", { style: { display: "inline-flex", alignItems: "center", gap: "6px", flexWrap: "wrap" } },
          ...
          [
            // [2026-09-01] 常驻服务优先（模型常驻内存，省掉每次加载模型的 20s+）；命令行只作兜底
            h("div", { style: { display: "flex", flexWrap: "wrap", gap: "10px" } },
              vField("常驻服务地址（推荐）", h("input", { value: eng.audio8?.url ?? "http://127.0.0.1:18795", onChange: (e) => setEngine("audio8", { url: e.target.value }, true), placeholder: "http://127.0.0.1:18795", style: vInput })),
              vField("命令行（兜底，一般不用改）", h("input", { value: eng.audio8?.cmd ?? "", onChange: (e) => setEngine("audio8", { cmd: e.target.value }, true), placeholder: "node C:\\D\\opt\\audio8-tts\\audio8-tts.mjs", style: vInput })),
            ),
            // 音色行：点名字=设为默认；🔊 原音=参考语音；🔊 克隆声=用该音色实时合成
            (() => {
              const voices = meta?.audio8Voices ?? [];
              const cur = eng.audio8?.voice ?? "";
              const row = (v) => h("div", { key: v || "__auto__", style: { display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap" } },
                h("button", {
                  type: "button", title: "设为默认音色",
                  style: {
                    border: "none", borderRadius: "6px", padding: "4px 10px", flex: "none", cursor: "pointer",
                    background: cur === v ? "rgba(229,72,77,.25)" : "rgba(128,128,128,.15)", color: "inherit", fontSize: "12px",
                  },
                  onClick: () => setEngine("audio8", { voice: v }, true),
                }, (cur === v ? "● " : "○ ") + (v === "" ? "自动（最新注册）" : v)),
                v === "" ? null : a8Btn("a8-src:" + v, "🔊 原音", () => playAudio8Source(v, "a8-src:" + v)),
                a8Btn("a8-clone:" + (v || "auto"), "🔊 克隆声（实时合成）", () => previewVoice("audio8", v || undefined, undefined, undefined, "a8-clone:" + (v || "auto"), { url: eng.audio8?.url ?? "", cmd: eng.audio8?.cmd ?? "" })),
              );
              return h("div", { style: { display: "flex", flexDirection: "column", gap: "6px" } },
                h("div", { style: { fontSize: "12px", color: "var(--dsw-alias-label-secondary,#9aa3ad)" } },
                  voices.length ? "已注册音色（点名字=设为默认；🔊 原音=参考语音，🔊 克隆声=实时合成）" : "暂无已注册音色（用 register_voice.py 或飞书配置中心上传一段参考语音）"),
                row(""),
                voices.map(row),
              );
            })(),
            h("div", { style: { fontSize: "12px", color: "var(--dsw-alias-label-secondary,#9aa3ad)" } }, "CPU 合成约 8-20 秒，点克隆声请稍等"),
          ]),

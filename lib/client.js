...
// 事件原生渲染）；rc.7 上 AI 语音音频已生成但界面不显示，属 rc.7 硬伤，引导用户使用源码版。
    ...
    function VideoReplyBar({ video, side }) {
      ...
      return h("div", {
        ...
        style: {
          ...
          // 钉死对齐：AI 左 / 用户右。外层 flex 容器拉满宽，靠 margin 推边。
    ...
    function videoPartsFromUserMessage(data) {
      ...
      for (const p of parts) {
        if (!p) continue;
        // 兼容多种附件形状：attachment 直挂 / attachment.file / p.file
        ...
        if (mt.indexOf("video/") === 0 || /\.(mp4|m4v|mov|webm)$/i.test(nm)) {
          ...
          // fileCard 点击灯箱用：文件名 → attachmentId
          try {
    ...
    function UserVideoNodeView({ node }) {
      ...
      // 用户侧：整行拉满，内容钉右（与用户气泡同侧）。
      return h("div", {
        "data-video-reply-row": "user",
        style: {
      ...
      }, vids.map((v) => h(VideoReplyBar, { key: v.attachmentId, video: v, side: "user" })));
    }
    ...
    function ToolbarLeft({ connection, sessionId }) {
      ...
      const syncOptProbe = () => {
        try {
          ...
          const mark = ` ⚡>${optStateNowRef.current} txt=${tl} pair=${pl} eq=${same ? "Y" : "N"} « ${optTraceRef.current.slice(-3).join(" ")}`;
          const base = String(document.title || "").replace(/ ⚡>[\s\S]*$/, "");
          if (document.title !== base + mark) document.title = base + mark; // harness 会自己改标题，这里补写回去
        } catch { /* 探针写不进标题不影响任何功能 */ }
      };
      ...
      useEffect(() => {
        const iv = window.setInterval(() => {
          ...
          syncOptProbe(); // 探针自愈：harness 改标题后 0.8 秒内补写回来，现场不会丢
          // [2026-09-25 查「还原」被误清] findComposerEl 会退化到 document.querySelector("textarea")
    ...
    function retroAsk({ title, quote, note, stopOption, replyOption }) {
      return new Promise((resolve) => {
        ...
        if (replyOption) {
          ...
          const sp2 = document.createElement("span"); sp2.textContent = "连模型对这条的回话一起删（它当时答的内容一并抹掉）"
          lab2.append(cb2, sp2)
    ...
    const RETRO_STAGES = {
      queue: {
        title: "抽回这条？",
        note: "这条还排在队列里，模型一个字都没看到。抽走就当没发过，不留痕迹。只动这一条。",
        stop: false,
      },
      running: {
        title: "撤回这条消息？",
        note: "这句已经发给模型了，字已经计过钱。删它保证往后的每句话它都看不到，本轮已经在跑的回答可以顺手叫停；界面上这条一并消失。已经让它办的事（改文件、发消息）不会因此撤销。只删你这一条，模型这轮答出去的内容保留。",
        stop: true,
      },
      done: {
        title: "撤回这条消息？",
        note: "这段已经聊完了。删它是把这句从后续每次提问的记忆里拿掉，界面上也一并消失。当时让它办出去的事不会因此撤销。只删你这一条，模型当时的回话保留。",
        stop: false,
      },
      compacted: {
        title: "撤回这条消息？",
      // [本地改造 2026-09-24 老大质疑后更正] 旧文案写"早就被折叠过"是黑话而且说死了原因：代码只查出
      // "这条不在当前送进模型的上下文里"，原因可能是压缩摘要顶掉了、也可能是超出上下文窗口。
      // 实测本会话：模型上下文只覆盖 seq 1709–2118，你 09-24 的第一句（seq 9）确实不在其中，
      // 所以"模型看不到"这个结论成立，措辞改成下面这样，不再谎称一定是折叠。
        note: "这条被摘要顶掉了：模型那边只剩几段摘要，原文早就不带着了。删它等于顺手把界面这一行也清掉。只删你这一条。",
        stop: false,
        reply: true,
      },
      // [本地改造 2026-09-24 老大] 宿主端现在能分清三件事，不再统一糊成"折叠过"：
      // 真被摘要顶掉(compacted) / 只是这次没带进上下文(windowed) / 这个序号根本不是消息(not-message)。
      windowed: {
        title: "撤回这条消息？",
        note: "这条这次没被带进模型的上下文（太久远，只带了最近若干条），所以现在删它对模型没影响；删掉是让它以后也不会被重新带回去。只删你这一条。",
        stop: false,
        reply: true,
      },
      'not-message': {
        title: "撤回这条消息？",
        note: "认不清这条现在算什么状态：它不在模型带的上下文里，服务端也没按一条消息认到它。删它会把界面这一行收掉，之后模型也不会再看到它。",
        stop: false,
      },
      // 内容核对没过：宁可什么都不动，也不能拿这个序号去删别的内容。
      mismatch: {
        title: "没敢删",
        note: "这一行的文字和会话里对上的记录不一致，说明我认错了行。为防删错别条，这次什么都没动。刷新页面再点一次试试。",
        stop: false,
      },
      retracted: {
        title: "这条已经撤回过了",
    ...
    async function retroDeleteMessage(sessionId, addr, preview) {
      ...
      const ans = await retroAsk({ title: stage.title, quote: preview, note: stage.note, stopOption: stage.stop, replyOption: stage.reply })
      if (!ans.ok) return
      const j = await retroApi("/retro-message-delete", {
        sessionId, seq: addr.seq, mid: addr.mid, turn: addr.turn, occurrence: addr.occurrence, messageId: addr.messageId, alsoStop: ans.alsoStop, alsoReply: ans.alsoReply === true, text: preview,
      })
      ...
      retroToast(j.mode === "queue" ? "已抽回，模型没看到过这条"
        : voided > 1 ? "已抹掉这条和它的回话（共 " + voided + " 项）" : "已从模型记忆里抹掉这条")
      retroScan()
    }
    ...
    function retroScan() {
      ...
      for (const row of document.querySelectorAll("[data-chat-flow-kind='user'],[data-chat-flow-kind='steering']")) {
        ...
        const text = (host.innerText ?? "").slice(0, 120)
        ...
        if ((addr.seq !== undefined && gone.has(addr.seq)) || (addr.mid !== undefined && goneIds.has(addr.mid))) {
        ...
        retroMountBtn(seat, "msg-" + rowId, "撤回这条", () => {
          if (sessionId === undefined) { retroToast("认不出当前是哪段会话，先别删", true); return }
          void retroDeleteMessage(sessionId, addr, text)
        })
      }
    ...
    function apply(ctx) {
      ...
      // [2026-09-24 修·回滚] 绝不隐藏官方视频 fileCard——上一版误藏导致用户视频
      // 「预览和附件都没了」。fileCard 一律保留；能匹配到 attachmentId 时点击走灯箱。
      // user-video 预览条能渲染就多一条（右对齐），渲染不出来至少还有 chip。
      ctx.effect(() => {
        ...
        // 把上一版误藏的 chip 全部放出来
        document.querySelectorAll('[class*="fileCard"][title]').forEach((el) => {
          if (el.dataset.videoChipHidden === "1" || el.style.display === "none") {
            el.style.display = "";
        ...
        document.addEventListener("click", (ev) => {
          ...
          const id = map && (map.get(title) || map.get(title.toLowerCase()));
          if (!id) return;
          ev.preventDefault();
          ev.stopPropagation();
          openPresentedMediaOverlay(videoSrcOf(id), title);
        }, true);
        return () => { /* document 级监听随页面生命周期 */ };

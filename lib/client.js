...
// 事件原生渲染）；rc.7 上 AI 语音音频已生成但界面不显示，属 rc.7 硬伤，引导用户使用源码版。
    ...
    const RETRO_STAGES = {
      queue: { title: "删除这条消息？", note: "还没发出去" },
      running: { title: "删除这条消息？", note: "正在答", stop: true },
      done: { title: "删除这条消息？", note: "答完了", reply: true },
      compacted: { title: "删除这条消息？", note: "不在上下文里", reply: true },
      windowed: { title: "删除这条消息？", note: "不在上下文里", reply: true },
      'not-message': { title: "删除这条消息？", note: "状态未知", reply: true },
      unknown: { title: "删除这条消息？", note: "状态未知", reply: true },
      mismatch: { title: "没敢删", note: "内容对不上" },
      retracted: { title: "已经删过了", note: "" },
    }
    ...
    function retroScan() {
      ...
      for (const host of document.querySelectorAll("[data-pending-id]")) {
        ...
        if (mid === "") continue
        const seat = host.querySelector("[class*='actions']") ?? host
        retroMountBtn(seat, "pend-" + mid, "删除这条（还没发出去）", () => {
          const sid = retroCurrentSessionId()
          if (sid === undefined) { retroToast("认不出这是哪段会话（刷新一下页面，或展开左侧历史列表再点）", true); return }

...
// 事件原生渲染）；rc.7 上 AI 语音音频已生成但界面不显示，属 rc.7 硬伤，引导用户使用源码版。
    ...
    function injectImageCard(anchorEl, path) {
      try {
        let host = anchorEl;
        for (let i = 0; i < 3 && host.parentElement; i++) host = host.parentElement;
        if (!host || host.querySelector("img[data-img-card]")) return;
        const wrap = document.createElement("div");
        wrap.style.cssText = "margin:6px 0;display:inline-flex;";
        const img = document.createElement("img");
        img.setAttribute("data-img-card", "1");
        img.src = "/vision/image?path=" + encodeURIComponent(path);
        img.alt = "图片";
        img.title = "点击放大";
        img.style.cssText = "max-width:260px;max-height:200px;border-radius:8px;border:1px solid rgba(128,128,128,.3);cursor:zoom-in;display:block;background:rgba(128,128,128,.08);";
        img.onclick = () => openImageZoom(path);
        wrap.appendChild(img);
        // 插到锚文本之后（作为其后兄弟，视觉上位于文本下方）
        const holder = host.querySelector("p, div") ?? host;

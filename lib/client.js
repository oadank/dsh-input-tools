...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function startVoiceBubbleObserver() {
      ...
      const tryInject = () => {
        ...
        for (const el of els) {
          ...
          if (!directText.includes(VOICE_MSG_MARK)) continue;
          const meta = pendingVoiceQueue.shift();
          if (meta) injectVoiceCard(el, meta.voiceId, meta.ext);
    ...
    function injectVoiceCard(anchorEl, voiceId, ext) {
      try {
        let host = anchorEl;
        for (let i = 0; i < 7 && host.parentElement; i++) {
          host = host.parentElement;
          const cls = typeof host.className === "string" ? host.className : "";
          if (/message|msg-|bubble|chat/i.test(cls)) break;
        }
        if (host.querySelector("audio[data-voice-bubble]")) return;
        const audio = document.createElement("audio");
        audio.src = `/voice/outbox/${voiceId}.${ext || "webm"}`;
        audio.preload = "metadata";
        audio.dataset.voiceBubble = "1";
        const btn = document.createElement("button");
        btn.type = "button";
        btn.style.cssText = "display:inline-flex;align-items:center;justify-content:center;width:32px;height:32px;border:none;border-radius:50%;background:rgba(229,72,77,.18);color:#e5484d;cursor:pointer;font-size:14px;flex:none;line-height:1;";
        btn.textContent = "▶";
        ...
        dur.style.cssText = "font-size:11px;opacity:.75;min-width:28px;";
        audio.onloadedmetadata = () => {
          const s = Math.round(audio.duration || 0);
          dur.textContent = s ? `${Math.floor(s / 60)}:${String(s % 60).padStart(2, "0")}` : "";
        };
        const card = document.createElement("div");
        card.style.cssText = "display:inline-flex;align-items:center;gap:8px;background:rgba(229,72,77,.1);border:1px solid rgba(229,72,77,.28);border-radius:999px;padding:5px 12px;margin-top:6px;";
        card.append(btn, dur);
      ...
      return h("div", { style: { display: "flex", flexDirection: "column", gap: "14px", padding: "16px", width: "100%", boxSizing: "border-box" } },
        ...
        caps !== null ? h("div", { style: { border: "1px solid var(--dsw-alias-border-l1,#333a45)", borderRadius: "10px", padding: "10px 12px", display: "flex", flexDirection: "column", gap: "6px", background: "rgba(128,128,128,.05)", fontSize: "12.5px", lineHeight: "1.5" } },
          ...
          caps.voiceContentContract === true ? null : h("div", { style: { fontSize: "12px", opacity: .75, marginTop: "2px" } },
            "说明：当前 dsh 版本（npm 安装版）契约不支持原生语音消息，语音会自动转成文字发送，AI 通过【用户语音】标记识别；如需原生语音消息体验，请使用源码版 dsh。"),
        ) : null,
        // ⑤ ASR 语音识别（必填项，无开关）

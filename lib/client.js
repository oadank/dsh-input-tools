...
// 事件原生渲染）；rc.7 上 AI 语音音频已生成但界面不显示，属 rc.7 硬伤，引导用户使用源码版。
    ...
    // [2026-08-22] 聊天图片卡渲染：识别「[图片]路径[/图片]」文本消息 → 渲染缩略图(可点击放大)
    // 图片入口拦截后消息里是路径文本（无 image 块），靠本观察器补回图片展示。
    let imgCardStarted = false;
    const injectedImgEls = new WeakSet();
    function startImageCardObserver() {
      if (imgCardStarted || typeof MutationObserver === "undefined") return;
      imgCardStarted = true;
      const tryInject = () => {
        const els = Array.from(document.querySelectorAll("div,span,p,li"));
        for (const el of els) {
          if (injectedImgEls.has(el)) continue;
          const text = el.textContent ?? "";
          if (!text.includes("[图片]")) continue;
          // 叶子级：子元素已含标记则跳过（父容器）
          let childHas = false;
          for (const c of el.children) {
            if ((c.textContent ?? "").includes("[图片]")) { childHas = true; break; }
          }
          if (childHas) continue;
          // [2026-08-22 修2] 持续注入并清空标记文本：即使 chip 已存在也要再清一遍
          //（React 重渲染会恢复 nodeValue，这里确保路径文本常驻为空，框选复制不到）
          injectImageCard(el);
          injectedImgEls.add(el);
        }
      };
      const obs = new MutationObserver(() => tryInject());
      obs.observe(document.body, { childList: true, subtree: true, characterData: true });
    ...
    // [2026-08-22] 解析标记内容 → 图片引用 URL：短名(img-xxx.jpg)走 ?name=，完整路径走 ?path=（旧消息兼容）
    const imageSrcFor = (ref) => {
      const s = (ref ?? "").trim();
      if (s === "") return "";
      if (/^img-[\w.-]+$/.test(s)) return "/vision/image?name=" + encodeURIComponent(s);
      return "/vision/image?path=" + encodeURIComponent(s);
    };
    // 构造原子芯片（contenteditable=false + user-select:none，选中/删除是一个整体）
    function buildImageChip(ref) {
      const chip = document.createElement("span");
      chip.setAttribute("data-img-chip", "1");
      chip.setAttribute("contenteditable", "false");
      chip.style.cssText = "display:inline-flex;align-items:center;gap:8px;margin:2px 6px;padding:4px 12px 4px 6px;border-radius:14px;background:rgba(91,156,255,.12);border:1px solid rgba(91,156,255,.32);color:var(--dsw-alias-link,#5b9cff);font-size:13px;cursor:zoom-in;user-select:none;-webkit-user-select:none;vertical-align:middle;max-width:320px;";
      const thumb = document.createElement("img");
      thumb.src = imageSrcFor(ref);
      thumb.style.cssText = "width:32px;height:32px;object-fit:cover;border-radius:8px;flex:none;background:rgba(128,128,128,.15);";
      const label = document.createElement("span");
      const fname = (ref ?? "").replace(/^.*[\\/]/, "");
      label.textContent = "🖼️ " + fname;
      label.style.cssText = "overflow:hidden;text-overflow:ellipsis;white-space:nowrap;min-width:0;flex:1 1 auto;font-family:monospace;pointer-events:none;";
      chip.appendChild(thumb);
      chip.appendChild(label);
      chip.title = "点击放大预览";
      chip.onclick = () => openImageZoom(ref);
      return chip;
    }

    // [2026-08-22 修] 图片 chip 注入：绝不 removeChild/replaceChild React 管理的节点！
    // 崩溃根因: 之前 tn.parentNode.replaceChild(frag, tn) 把 React 管理的文本节点移出 DOM,
    // React 重渲染时 removeChild 该节点 → NotFoundError → 'conversation.composer.bar' 槽崩溃 → 聊天框消失。
    // 正解: 在文本节点前 insertBefore 插入 chip(兄弟节点), 原文本节点保留在树上(nodeValue 清空),
    // React 的虚拟 DOM 与真实 DOM 结构始终一致, 重渲染不会崩。
    // [2026-08-22 修2] React 重渲染会把 nodeValue 恢复 → 标记文本重现(可被框选复制)。
    // 因此每次 tryInject 都清空标记文本(即使 chip 已存在), 让路径文本常驻为"已消费"状态。
    function injectImageCard(anchorEl) {
      try {
        const walker = document.createTreeWalker(anchorEl, NodeFilter.SHOW_TEXT);
        const textNodes = [];
        let node;
        while ((node = walker.nextNode())) {
          if ((node.nodeValue ?? "").includes("[图片]")) textNodes.push(node);
        }
        if (textNodes.length === 0) return;
        for (const tn of textNodes) {
          const value = tn.nodeValue ?? "";
          const parts = value.split(/(\[图片\][^\[]*?\[\/图片\])/g);
          if (parts.length === 1) continue;
          const frag = document.createDocumentFragment();
          let hasChip = false;
          for (const part of parts) {
            if (part === "") continue;
            const m = /^\[图片\](.*)\[\/图片\]$/.exec(part);
            if (m) {
              const p = (m[1] ?? "").trim();
              if (p !== "") { frag.appendChild(buildImageChip(p)); hasChip = true; }
          ...
          if (!hasChip) continue;
          // 把 chip(及剩余文本)插到原文本节点前面作为兄弟；原节点保留、值清空 —— 不破坏 React 树结构
    ...
    function openImageZoom(ref) {
      try {
        const mask = document.createElement("div");
        mask.style.cssText = "position:fixed;inset:0;z-index:9999;background:rgba(0,0,0,.78);display:flex;align-items:center;justify-content:center;padding:24px;";
        const img = document.createElement("img");
        img.src = imageSrcFor(ref);
        img.style.cssText = "max-width:92vw;max-height:88vh;object-fit:contain;border-radius:10px;box-shadow:0 12px 48px rgba(0,0,0,.6);cursor:zoom-out;";
        img.title = "点击缩小";
        ...
        btn.textContent = "✕";
        btn.style.cssText = "position:fixed;top:16px;right:16px;border:none;border-radius:999px;width:34px;height:34px;background:rgba(229,72,77,.9);color:#fff;font-size:16px;cursor:pointer;";
        btn.onclick = close;
    ...
    function ToolbarLeft({ connection, sessionId }) {
      ...
      // [2026-08-22 修] 记住插入标记文本的 textarea（删除时精确用它，防止 querySelector 选错元素导致 React 崩溃）
      const visionComposerRef = useRef(null);
      // [2026-08-22] 图片入口拦截：原图上传(不走官方 draft/无 2000px 限制) → 短名标记进输入框
      // [2026-08-22 修] 消息里只放短文件名(name)，不放完整路径——避免聊天框选复制到 C:\... 长路径
      const [visionDraftName, setVisionDraftName] = useState(null); // 当前已选图的短文件名（img-xxx.jpg）
      ...
      // [2026-08-22] 图片卡注入：识别 [图片]路径[/图片] 文本消息渲染缩略图
      useEffect(() => { startVoiceBubbleObserver(); startImageCardObserver(); }, []);

      // [2026-08-22] 把路径文本插入官方输入框（modlens insertText 技巧：execCommand 触发 React input 事件）
      // ⚠️ [2026-08-22 修] 只用 execCommand；绝不 setter 改 value 兜底（React 受控组件会崩）
      const insertIntoComposer = (text) => {
        const el = typeof document !== "undefined" ? document.querySelector("textarea") : null;
        if (!el) return false;
        visionComposerRef.current = el; // [2026-08-22 修] 记住插入的元素，删除时用它（避免 querySelector 选错）
        el.focus();
        try {
      ...
      // [2026-08-22] 图片上传：原图 base64 → POST /vision/upload → 存 DSH_HOME/vision-uploads → 路径文本进输入框
      const uploadImage = async (file) => {
        ...
        try {
          ...
          const resp = await fetch("/vision/upload", {
            method: "POST", headers: { "content-type": "application/json" },
            body: JSON.stringify({ name: file.name, data: b64 }),
          });
          const d = await resp.json();
          if (d?.ok && typeof d.name === "string") {
            setVisionDraftName(d.name);
            insertIntoComposer("\n[图片]" + d.name + "[/图片]\n");
          } else {
            setVisionUploading(false);
            setVoiceError(d?.error ?? "图片上传失败");
          }
        } catch (e) {
          setVisionUploading(false);
          setVoiceError("图片上传失败：" + String(e?.message ?? e));
          return;
        }
        setVisionUploading(false);
      };

      // [2026-08-22] 从输入框移除指定文本（× 关闭预览时删除已插入的 [图片]...[/图片] 标记）
      // ⚠️ [2026-08-22 修] 官方 textarea 是 React 受控组件：绝不直接改 value / 绝不 setter 兜底
      //（React value-tracker 检测到 DOM 值被外部修改会抛错 → 整棵组件树卸载 → 聊天框消失）。
      // 只用 execCommand('delete') 模拟用户操作（与插入的 insertText 同一被验证安全的路线）；
      // 失败就什么都不做（宁可标记文本留着，绝不崩）。
      const removeFromComposer = (text) => {
        if (text === "" || typeof document === "undefined") return;
        try {
          // 优先用插入时记住的元素；失效则按「value 包含该标记」精确查找，绝不误改其他 textarea
          let el = visionComposerRef.current;
          if (el === null || !document.contains(el) || !((el.value ?? "").includes(text))) {
            el = null;
            for (const t of Array.from(document.querySelectorAll("textarea"))) {
              if ((t.value ?? "").includes(text)) { el = t; break; }
            }
          }
          if (el === null) return;
          const v = el.value;
          ...
          if (idx === -1) return;
          // 模拟用户操作：聚焦 → 选中标记文本 → execCommand 删除（触发 React onChange，状态一致）
          el.focus();
          el.setSelectionRange(idx, idx + text.length);
          if (document.execCommand("delete", false)) return; // 成功即返回；失败静默（不碰 value）
        } catch { /* 忽略：绝不崩 */ }
      };
      // [2026-08-22] 关闭图片预览：清掉已选图 + 删除输入框里的标记文本（两者绑定）
      const clearVisionDraft = () => {
        const n = visionDraftName;
        if (n !== null) removeFromComposer("\n[图片]" + n + "[/图片]\n");
        setVisionDraftName(null);
      };

      // [2026-08-22] Ctrl+V 粘贴图片拦截（capture 阶段，官方 draft 之前）→ 走入口拦截上传
      // 官方 InputBar 的 onPaste → intakeImages 会把图挂到 draft（显示在聊天上方、受 2000px 限制）
      useEffect(() => {
        const onPaste = (e) => {
          try {
      ...
      const sendVoiceBlob = useCallback(async (blob) => {
        ...
        // [2026-08-22 改] 图片入口拦截：已选图 = 短名标记文本块（原图已传 /vision/upload，无 2000px 限制）
        const draftImageContents = async () => {
          if (visionDraftName === null || visionDraftName === "") return [];
          return [{ type: "text", text: "[图片]" + visionDraftName + "[/图片]" }];
        };
        // [2026-08-21] 语音发送成功后清掉已选图（含输入框标记文本）
      ...
      // [2026-08-22 改] 图片选中 → 原图上传（入口拦截，无 2000px 限制）→ 路径文本进输入框
      ...
      return h("div", { style: { position: "relative", display: "inline-flex", alignItems: "center", gap: "10px" } },
        h("button", {
        ...
        // [2026-08-22] 已选图片预览（缩略图可点击放大；✕ 关闭时同时删除输入框里的标记文本）
        visionDraftName !== null ? h("span", { style: { position: "relative", display: "inline-flex" } },
          h("img", {
            src: "/vision/image?name=" + encodeURIComponent(visionDraftName),
            alt: "已选图片",
            title: "点击放大预览",
            onClick: () => openImageZoom(visionDraftName),
            style: { width: "36px", height: "36px", objectFit: "cover", borderRadius: "6px", border: "1px solid var(--dsw-alias-border-l1,#333a45)", cursor: "zoom-in" },
          }),
          h("button", {
            type: "button", "aria-label": "移除已选图片", title: "移除已选图片",
            style: {
              position: "absolute", top: "-6px", right: "-6px",
              border: "none", borderRadius: "999px", width: "16px", height: "16px",
              background: "rgba(229,72,77,.9)", color: "#fff", fontSize: "10px", cursor: "pointer",
              display: "flex", alignItems: "center", justifyContent: "center", padding: "0",
            },
            onMouseDown: (e) => e.preventDefault(),
            onClick: () => clearVisionDraft(),
          }, "✕"),
        ) : null,

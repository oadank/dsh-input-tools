...
// 事件原生渲染）；rc.7 上 AI 语音音频已生成但界面不显示，属 rc.7 硬伤，引导用户使用源码版。
    ...
    function retroScan() {
      ...
      for (const row of document.querySelectorAll("[data-chat-flow-kind='user'],[data-chat-flow-kind='steering']")) {
        ...
        if (goneByText || (addr.seq !== undefined && gone.has(addr.seq)) || (addr.mid !== undefined && goneIds.has(addr.mid))) {
      ...
      for (const host of document.querySelectorAll("[data-chat-node-key]")) {
        ...
        const dead = (/^\d+$/.test(tail) ? gone.has(Number(tail)) : goneIds.has(tail))
          || (kind === "turn-process" && Number.isSafeInteger(procTurn) && procTurn >= 0 && deadTurns.has(procTurn))
        if (dead) {
      ...
      // [本地改造 2026-09-24 老大] 侧栏会话行的「删除这段历史」按钮已摘掉：那一列本来就有系统的      if (!RETRO.domReported && sessionId !== undefined) {
        RETRO.domReported = true
        const dump = []
        for (const probeRow of document.querySelectorAll("[data-chat-flow-kind='user'],[data-chat-flow-kind='steering']")) {
          ...
          dump.push({ key: String(ph.getAttribute("data-chat-node-key") ?? ""), kind: String(ph.getAttribute("data-chat-flow-kind") ?? ""), turn: String(ph.getAttribute("data-chat-turn") ?? "") })
          if (dump.length >= 4) break
        }
        if (dump.length > 0) void retroApi("/retro-dom", { sessionId, dump })
      }
    ...
    function apply(ctx) {
      ...
      ctx.effect(() => {
        ...
        const disposers = [
          ...
          safeInject("settings.section", () => ctx.slots.register({
          ...
          safeInject("conversation.chat.voice-actions", () => ctx.slots.register({
              ...
              dispose = commandUi.register({
                ...
                ui: {
                  ...
                  options: async (session) => {
                    ...
                    try {
                      if (!document.getElementById("dsh-de-wrap")) {
                        const st = document.createElement("style");
                        st.id = "dsh-de-wrap";
                        st.textContent = '[role="listbox"] [class*="label"],[role="listbox"] [class*="detail"]{white-space:normal!important;text-overflow:clip!important;overflow:visible!important;}'
                          + '[role="listbox"] [class*="row"]{height:auto!important;min-height:0!important;padding-top:5px;padding-bottom:5px;line-height:1.4;}';
                        document.head.appendChild(st);
                      }
                    } catch { /* 样式注入失败不拦候选 */ }
                    ...
                    return list.map((c, i) => {
                      ...
                      const pv = Number(c && c.p);
                      return {
                        id: t,
                        // 🔴 全文进 label，一个字符都不删（"看不到全文就没法判断用不用"）
                        label: t,
                        detail: (c && c.role ? c.role + "｜" : "")
                          + "拟用 " + (Number.isFinite(pv) ? Math.round(pv * 100) + "%" : "—")
                          + (j.judge && j.judge.risk ? "｜风险 " + j.judge.risk : "")
                          + (j.judge && j.judge.scope ? "｜量级 " + j.judge.scope : "")
                          + "｜" + thick,
                        badge: c && c.role ? c.role : (i === 0 && Number.isFinite(pv) && pv >= 0.6 ? "荐" : undefined),
                        active: i === 0,
                      };
                    });
                  },

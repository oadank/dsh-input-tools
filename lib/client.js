...
// 事件原生渲染）；rc.7 上 AI 语音音频已生成但界面不显示，属 rc.7 硬伤，引导用户使用源码版。
    ...
    function apply(ctx) {
      ...
      ctx.effect(() => {
        const disposers = [
          // [2026-09-20] 视频消息：conversation node + keyed chat 渲染器（详见上方视频段注释）
          (() => {
            try {
              const uic = typeof ctx.get === "function" ? ctx.get("uiConversation") : null;
              return uic && uic.events ? uic.events.register(videoReplyDefinition) : () => {};
            } catch { return () => {}; }
          })(),
          (() => {
            try {
              const uic = typeof ctx.get === "function" ? ctx.get("uiConversation") : null;
              return uic && uic.events ? uic.events.register(userVideoDefinition) : () => {};
            } catch { return () => {}; }
          })(),
          ctx.slots.inject("conversation.chat.node", () => ctx.slots.register({
            name: "conversation.chat.node", key: "video-reply", locale: "conversation",
          }, VideoReplyNodeView)),
          ctx.slots.inject("conversation.chat.node", () => ctx.slots.register({

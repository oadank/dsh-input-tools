  ...
  factory: (require) => {
    ...
    function ToolbarLeft({ connection, sessionId }) {
      ...
      const sendVoiceBlob = useCallback(async (blob) => {
        ...
        try {
          // [2026-08-21 修] 录音先走本地 ASR 转文本再发送：XDN 的 npm dsh(rc.7) 的
          // session.prompt 契约不支持 content type="voice"（多模态直发），会报
          // "invalid payload for session.prompt"。统一改 ASR 转文本（text 契约所有版本
          // 通用），本机 lecoo(rc.8) 行为一致。ASR 未配置/识别失败会给出明确提示。

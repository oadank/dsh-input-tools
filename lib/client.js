...
// 事件原生渲染）；rc.7 上 AI 语音音频已生成但界面不显示，属 rc.7 硬伤，引导用户使用源码版。
    ...
    function VisionSettingsSection() {
      ...
      const [visionEditMode, setVisionEditMode] = useState("view"); // [2026-08-22] view=查看 / edit=编辑 / saved=已保存
      ...
      return h("div", { style: { display: "flex", flexDirection: "column", gap: "14px", padding: "16px", width: "100%", boxSizing: "border-box" } },
        ...
        h("div", { style: { display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap", fontSize: "15px", fontWeight: 700, color: "var(--dsw-alias-label-primary,#e6e9ef)" } },
          ...
          helpTip(
            "识图工具 look_image 三种模式：\n"
              + "· describe = 看图描述（默认，一两句简要）\n"
              + "· reverse = 像素级反推生图提示词（输出较长）\n"
              + "· text = 逐字提取图中文字\n"
              + "模型收到图片后会按提问自动选择模式调用。\n\n"
              + "配置：本地部署（ollama / sglang / vllm 等，本机起的 OpenAI 兼容 /v1 端点，无需 Key）或在线云端 API（填地址 + API Key）。\n"
              + "下方可测试配置是否连通，也可查看/编辑每个模式的提示词。",
        ...
        // [2026-08-22] 提示词弹窗：默认查看模式，可一键复制；点「编辑」→保存/恢复默认；保存后短暂显示「已保存」
        visionEditKey !== null ? h("div", {
          ...
          h("div", { style: {
            ...
            h("div", { style: { display: "flex", alignItems: "center", gap: "8px", padding: "10px 14px", borderBottom: "1px solid var(--dsw-alias-border-l1,#333a45)", fontSize: "13px", fontWeight: 600 } },
              ...
              h("span", { style: { marginLeft: "auto", display: "flex", gap: "6px" } },
                visionEditMode === "view"
                  ? h("button", { type: "button", style: btnSmall, onMouseDown: (e) => e.preventDefault(), onClick: () => copyText(visionEditDraft) }, "一键复制")
                  : null,
                visionEditMode === "view"
                  ? h("button", { type: "button", style: { ...btnSmall, background: "rgba(91,156,255,.2)", color: "var(--dsw-alias-link,#5b9cff)" }, onMouseDown: (e) => e.preventDefault(), onClick: () => setVisionEditMode("edit") }, "编辑")
            ...
            visionEditMode === "view"
              ...
              }, visionEditDraft !== "" ? visionEditDraft : "（未自定义，使用内置默认提示词）")
              : h("textarea", {
            ...
            h("div", { style: { padding: "0 14px 10px", display: "flex", alignItems: "center", justifyContent: "flex-end", gap: "8px", fontSize: "11px", opacity: .75 } },
              visionEditMode === "edit"
                ...
                : h("span", {}, visionEditMode === "view" ? "点「编辑」可修改提示词" : "已保存到配置"),
            ),
          ),
        ) : null,

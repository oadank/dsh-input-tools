...
// 事件原生渲染）；rc.7 上 AI 语音音频已生成但界面不显示，属 rc.7 硬伤，引导用户使用源码版。
    ...
    function startImageCardObserver() {
      ...
      const tryInject = () => {
        ...
        for (const el of els) {
          ...
          if (el.querySelector("[data-img-chip]")) { injectedImgEls.add(el); continue; }
          const text = el.textContent ?? "";
    ...
    function buildImageChip(path) {
      ...
      thumb.src = "/vision/image?path=" + encodeURIComponent(path);
      ...
      chip.title = "点击放大预览";
      chip.onclick = () => openImageZoom(path);
      return chip;
    }
    ...
    function injectImageCard(anchorEl) {
      try {
        if (anchorEl.querySelector("[data-img-chip]")) return;
    ...
    function openImageZoom(path) {
      try {
        ...
        img.src = "/vision/image?path=" + encodeURIComponent(path);
    ...
    function ToolbarLeft({ connection, sessionId }) {
      ...
      // [2026-08-22] 图片入口拦截：原图上传(不走官方 draft/无 2000px 限制) → 路径文本进输入框
      const [visionDraftPath, setVisionDraftPath] = useState(null); // 当前已选图的本地路径
      ...
      const uploadImage = async (file) => {
        ...
        try {
          ...
          if (d?.ok && typeof d.path === "string") {
            setVisionDraftPath(d.path);
            insertIntoComposer("\n[图片]" + d.path + "[/图片]\n");
          } else {
      ...
      const clearVisionDraft = () => {
        const p = visionDraftPath;
        if (p !== null) removeFromComposer("\n[图片]" + p + "[/图片]\n");
        setVisionDraftPath(null);
      };
      ...
      const sendVoiceBlob = useCallback(async (blob) => {
        ...
        // [2026-08-22 改] 图片入口拦截：已选图 = 路径文本块（原图已传 /vision/upload，无 2000px 限制）
        const draftImageContents = async () => {
          if (visionDraftPath === null || visionDraftPath === "") return [];
          return [{ type: "text", text: "[图片]" + visionDraftPath + "[/图片]" }];
        };
      ...
      return h("div", { style: { position: "relative", display: "inline-flex", alignItems: "center", gap: "10px" } },
        ...
        visionDraftPath !== null ? h("span", { style: { position: "relative", display: "inline-flex" } },
          h("img", {
            src: "/vision/image?path=" + encodeURIComponent(visionDraftPath),
            alt: "已选图片",
            title: "点击放大预览",
            onClick: () => openImageZoom(visionDraftPath),

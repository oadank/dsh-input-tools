      ...
      return h("div", { ref: rootRef, "data-composer-left": "", style: { position: "relative", display: "inline-flex", alignItems: "center", gap: "12px" } },
        ...
        (() => {
          ...
          const onOptimize = async () => {
            ...
            if (!ta) { console.error("[dsh-optimize] textarea not found"); return; }
            const cur = String(ta.value || "").trim();
            const pair = optPairRef.current;
            if (optState === "done" && pair && cur === pair.optimized) {
              setNativeValue(ta, pair.original); // done 态再点 → 还原原文
              optPairRef.current = null;
              setOptState("idle");
              return;
            }
            optPairRef.current = null; // 手改过/无成对：以当前文本重新优化
            if (cur === "") return;
            setOptState("busy");
            try {

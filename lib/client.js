...
// 事件原生渲染）；rc.7 上 AI 语音音频已生成但界面不显示，属 rc.7 硬伤，引导用户使用源码版。
    ...
    function ToolbarLeft({ connection, sessionId }) {
      ...
      // [2026-08-27] 助手语音自动播放：DOM 检测 + 滑动静默期（重开不重播历史，新语音自动播）
    ...
    function BalanceMeter({ connection, sessionId, useSession, getModelStore }) {
      ...
      const refresh = useCallback(async () => {
        ...
        try {
          ...
          setVisible(value.balance !== null || value.usage !== null);
        } catch { /* 静默 */ }
      }, [connection, sessionId]);
      // ① 挂载 / 切会话时查一次
      ...
      // ③ 切换模型 / 新一轮请求查一次：订阅 mux 会话事件流（独立 SSE，自管重连）
      useEffect(() => {
        if (connection === undefined) return;
          ...
          while (!stopped) {
            try {
              ...
              for await (const env of stream) {
                ...
                if (!ev) continue;
                if (ev.type === "request/header" && ev.reason === "change") void refresh();
                else if (ev.type === "request/context") void refresh();
                // [本地改造 2026-08-25 补] 长任务实时刷新：每次 agent 回合结束(turn/end)都查一次余额。
                //  长任务(一条消息跑很多轮)中间 request/context 不常发，只在开始时查 → 余额偷跑不更新。
                //  turn/end 在每个 LLM 回合/工具轮结束后触发，输出 token 实时扣减 → 余额实时跟。
                //  频率无需客户端节流：host readGwBalance 已有 5s 缓存，频繁刷新自动折叠成最多 5s 一次。
                else if (ev.type === "turn/end") void refresh();
              }
            } catch { /* 流断，退避重连 */ }
      ...
      if (!visible || (balance === null && usage === null)) return null;
    ...
    function VoiceSettingsSection() {
      ...
      // [2026-09-01] 折叠块：标题行点击=选中该模式并展开，未选中的只留一行标题（老大：三个模式折叠起来，别一次全铺开）
      const vdSection = (isActive, title, onPick, body) => h("div", {
        style: {
          border: "1px solid " + (isActive ? "var(--vk-accent,#4b6fff)" : "var(--dsw-alias-border-l1,#333a45)"),
          borderRadius: "8px", background: isActive ? "rgba(75,111,255,.06)" : "transparent", overflow: "hidden",
        },
      },
        h("div", {
          style: { display: "flex", alignItems: "center", gap: "6px", padding: "7px 10px", cursor: "pointer", userSelect: "none" },
          onClick: onPick,
        },
          h("span", { style: { fontSize: "11px", width: "10px", flex: "none", color: isActive ? "var(--vk-accent,#4b6fff)" : "var(--dsw-alias-label-secondary,#9aa3ad)" } }, isActive ? "▾" : "▸"),
          h("span", { style: { fontSize: "12.5px", fontWeight: 600, color: isActive ? "var(--dsw-alias-label-primary,#e6e9ef)" : "var(--dsw-alias-label-secondary,#9aa3ad)" } }, title),
          isActive ? h("span", { style: { fontSize: "10.5px", color: "var(--vk-accent,#4b6fff)", flex: "none" } }, "使用中") : null,
        ),
        isActive ? h("div", { style: { padding: "2px 10px 8px", display: "flex", flexDirection: "column", gap: "6px" } }, body) : null,
      );
        ...
        vCard(h("span", { style: { display: "inline-flex", alignItems: "center", gap: "6px", flexWrap: "wrap" } },
          ...
          () => h("div", { style: { display: "flex", flexDirection: "column", gap: "10px" } },
            ...
            h("div", { style: { display: "flex", flexDirection: "column", gap: "8px", borderTop: "1px dashed var(--dsw-alias-border-l1,#333a45)", borderLeft: "2px solid #38bdf8", paddingTop: "8px", paddingLeft: "8px" } },
              ...
              // [2026-09-01] 三块折叠：官方示例 / 自定义 / 交给 AI，选中那块才展开（老大：别一次全铺开，示例前不要圆圈）
              vdSection(VD_KEYS.includes(vdMode), "官方示例",
                () => { if (!VD_KEYS.includes(vdMode)) pickVdMode("asmr"); },
                VOICE_DESIGN_EXAMPLES.map((ex, i) => {
                  ...
                  return h("div", {
                    ...
                    a8Btn("vd-ex:" + key, "🔊 试听", () => previewVoice("voicedesign", undefined, ex.instruct, undefined, "vd-ex:" + key, { text: ex.text })),
                  );
                })),
              vdSection(vdMode === "custom", "自定义音色描述", () => pickVdMode("custom"),
              ...
              vdSection(vdMode === "ai", "交给 AI 自动发挥", () => pickVdMode("ai"),
                h("div", { style: { display: "flex", flexDirection: "column", gap: "6px" } },
                  ...
                  h("div", { style: { display: "flex", alignItems: "center", gap: "8px" } },
                    ...
                    h("span", { style: { fontSize: "11.5px", color: "var(--dsw-alias-label-secondary,#9aa3ad)" } }, "每次随机一条音色描述，性别/年龄/质感按上面锁定"),
                  ),
                )),
            ),

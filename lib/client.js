...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // ── [2026-08-27] 助手语音自动播放（默认开）────
    // 按「已读/未读」实现：源码 VoiceReplyNodeView（本地改造）给语音卡暴露稳定的 data-voice-id，
    // 自动播放过的 voiceId 持久化到 localStorage → 重开页面历史语音（已播过）不重播，
    // 新语音一到立即自动播（无需点击、无需 mux）。用户自己的语音(data-voice)不自动播。
    ...
    const PLAYED_VOICE_KEY = "dsh.autoPlayedVoiceIds";
    ...
    function loadPlayedVoiceIds() {
      try {
        const raw = localStorage.getItem(PLAYED_VOICE_KEY);
    ...
    function startAssistantVoiceAutoPlay() {
      ...
      const played = loadPlayedVoiceIds();
      const pendingRetry = new Map(); // audio -> vid：被自动播放策略拦截，等下一次用户手势补播
      const playAudio = (audio, vid) => {
        if (lastAutoPlayedAudio !== null && lastAutoPlayedAudio !== audio && !lastAutoPlayedAudio.paused) {
          try { lastAutoPlayedAudio.pause(); } catch { /* 忽略 */ }
        }
        lastAutoPlayedAudio = audio;
        audio.play().then(() => {
          pendingRetry.delete(audio);
          // 只有真正播成功才标记「已播」；被拦的不标记，避免语音永远丢失
          if (vid) {
            played.add(vid);
            try { localStorage.setItem(PLAYED_VOICE_KEY, JSON.stringify(Array.from(played).slice(-300))); } catch { /* 忽略 */ }
          }
        }).catch(() => {
          // 被自动播放策略拦截（如 iOS 锁屏/后台后需新手势）：挂起，等下次用户手势补播
          if (vid) pendingRetry.set(audio, vid);
        });
      };
      const tryPlay = () => {
        ...
        for (const card of cards) {
          ...
          if (!vid || played.has(vid)) continue;
          const audio = card.querySelector("audio");
          if (!audio || !audio.src) continue; // audio 未就绪（异步 load），留待下次扫描
          playAudio(audio, vid);
        }
      };
      const retryPending = () => {
        if (!autoPlayAssistantVoice) return;
        for (const [audio, vid] of Array.from(pendingRetry.entries())) playAudio(audio, vid);
      };
      const obs = new MutationObserver(() => tryPlay());
      obs.observe(document.body, { childList: true, subtree: true, attributes: true, attributeFilter: ["src", "data-voice-id"] });
      window.setInterval(() => tryPlay(), 1000);
      // iOS：锁屏/后台后要求新的用户手势才允许有声自动播放 → 用户一点（触摸/按键）即补播被拦的语音
      document.addEventListener("pointerdown", retryPending, { capture: true });
      document.addEventListener("touchstart", retryPending, { capture: true });
...
// 事件原生渲染）；rc.7 上 AI 语音音频已生成但界面不显示，属 rc.7 硬伤，引导用户使用源码版。
    ...
    function ToolbarLeft({ connection, sessionId }) {
      ...
      const sendVoiceBlob = useCallback(async (blob) => {
        ...
        const sendAsVoice = async (images) => {
          // [0.1.5] 旧 connection.api.sessions.prompt 已迁 Typert Remote；无此 API 直接抛契约错走 ASR。
          const api = connection && connection.api && connection.api.sessions;
      ...
      const startRecording = useCallback(async () => {
        ...
        try {
          ...
          const gum = navigator && navigator.mediaDevices && typeof navigator.mediaDevices.getUserMedia === "function"
            ? navigator.mediaDevices.getUserMedia({ audio: true })
            : Promise.reject(new TypeError("undefined is not an object (evaluating 'navigator.mediaDevices.getUserMedia')"));
      ...
      return h("div", { ref: rootRef, "data-composer-left": "", style: { position: "relative", display: "inline-flex", alignItems: "center", gap: "16px" } },
        h("button", {
    ...
    function ComposerAttachmentsOverlay({ attachments, onAddFiles, onAddImages, onRemoveAttachment, onRemoveImage }) {
      ...
      const items = (Array.isArray(attachments) ? attachments : []).filter((a) => a?.kind !== "file");
    ...
    function McpSection() {
      ...
      const [form, setForm] = useState({ serverName: "", transport: "stdio", command: "", url: "", args: "" });
      const [status, setStatus] = useState(null);
      const reload = () => {
      ...
      const add = async () => {
        ...
        const ok = await post("/vscode-files/mcp/add", {
          serverName: form.serverName.trim(), transport: form.transport,
          command: form.command.trim(), url: form.url.trim(), args,
        });
        if (ok) { setAdding(false); setForm({ serverName: "", transport: "stdio", command: "", url: "", args: "" }); }
      };
      return h("div", { style: { display: "flex", flexDirection: "column", gap: "8px" } },
        h("div", { style: hostFilesHint }, "MCP 服务器（~/.dsh/mcp-servers.json）：启用即挂载进 agent 工具面；改动实时生效。"),
        servers === null
          ? h("div", { style: hostFilesHint }, "加载中…")
          : servers.length === 0
            ? h("div", { style: hostFilesHint }, "还没有 MCP 服务器，点下方「添加」。")
            : h("div", { style: { display: "flex", flexDirection: "column", gap: "6px" } },
              servers.map((s) => h("div", { key: s.id, style: hostFilesRow },
                h("button", {
                  type: "button", title: s.enabled ? "点击停用" : "点击启用",
                  style: { border: "none", borderRadius: "999px", width: "34px", height: "18px", padding: "0", cursor: "pointer", fontSize: "10px", fontWeight: 700, background: s.enabled ? "rgba(46,204,113,.35)" : "rgba(128,128,128,.25)", color: "inherit", flex: "none" },
                  onClick: () => post("/vscode-files/mcp/toggle", { id: s.id }),
                }, s.enabled ? "ON" : "OFF"),
                h("span", { style: { fontWeight: 600 } }, s.serverName),
                h("span", { style: { color: "var(--dsw-alias-label-secondary,#9aa3ad)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", flex: "1" } },
        ...
        adding
          ? h("div", { style: { display: "flex", flexDirection: "column", gap: "6px", padding: "10px", border: "1px dashed var(--dsw-alias-border-l1,#333a45)", borderRadius: "8px" } },
            ...
            form.transport === "stdio"
              ? h("div", { style: { display: "flex", gap: "8px" } },
                h("input", { placeholder: "command，如 node", value: form.command, onChange: (e) => setForm({ ...form, command: e.target.value }), style: { ...vInput, flex: "1" } }),
                h("input", { placeholder: "args（空格分隔）", value: form.args, onChange: (e) => setForm({ ...form, args: e.target.value }), style: { ...vInput, flex: "1" } }),
              )
              : h("input", { placeholder: "URL，如 http://127.0.0.1:3114/mcp", value: form.url, onChange: (e) => setForm({ ...form, url: e.target.value }), style: vInput }),
            h("div", { style: { display: "flex", gap: "8px" } },
              h("button", { type: "button", style: hostFilesBtn, onClick: add }, "添加"),
              h("button", { type: "button", style: hostFilesBtnGhost, onClick: () => setAdding(false) }, "取消"),
            ),
          )
          : h("div", null, h("button", { type: "button", style: hostFilesBtnGhost, onClick: () => setAdding(true) }, "＋ 添加 MCP 服务器")),
      );
    }
    ...
    function SkillsSection() {
      ...
      return h("div", { style: { display: "flex", flexDirection: "column", gap: "8px" } },
        h("div", { style: hostFilesHint }, "Skill（~/.dsh/skills/）：停用即改 .disabled，删除移入 .deleted（可手工恢复）。改动即时生效。"),
        skills === null
          ? h("div", { style: hostFilesHint }, "加载中…")
          : skills.length === 0
            ? h("div", { style: hostFilesHint }, "还没有 Skill。")
            : h("div", { style: { display: "flex", flexDirection: "column", gap: "6px" } },
              skills.map((s) => h("div", { key: s.path, style: hostFilesRow },
                h("button", {
                  type: "button", title: s.enabled ? "点击停用" : "点击启用",
                  style: { border: "none", borderRadius: "999px", width: "34px", height: "18px", padding: "0", cursor: "pointer", fontSize: "10px", fontWeight: 700, background: s.enabled ? "rgba(46,204,113,.35)" : "rgba(128,128,128,.25)", color: "inherit", flex: "none" },
                  onClick: () => post("/vscode-files/skills/toggle", { path: s.path }),
                }, s.enabled ? "ON" : "OFF"),
                h("span", { style: { fontWeight: 600 } }, s.name),
                h("span", { style: { color: "var(--dsw-alias-label-secondary,#9aa3ad)", flex: "1", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" } }, s.kind === "dir" ? "目录" : "单文件"),

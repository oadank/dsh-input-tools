...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function startVoiceBubbleObserver() {
      ...
      const tryInject = () => {
        ...
        for (const el of els) {
          ...
          // [2026-08-21] AI 语音回复：send_voice 工具输出"语音已发送（voiceId: sha256:...）" → 注入可播放语音条
          if (text.includes("语音已发送") && text.includes("voiceId") && text.includes("sha256:")) {
            let childHasMark = false;
            for (const c of el.children) {
              const ct = c.textContent ?? "";
              if (ct.includes("语音已发送") && ct.includes("sha256:")) { childHasMark = true; break; }
            }
            if (childHasMark) continue;
            const sha = text.match(/sha256:([0-9a-f]{64})/)?.[1];
            if (sha) { injectVoiceCard(el, `/voice/object/${sha}`); }
          ...
          if (meta) injectVoiceCard(el, `/voice/outbox/${meta.voiceId}.${meta.ext || "webm"}`);
          injectedVoiceEls.add(el);
        }
      };
    ...
    function injectVoiceCard(anchorEl, audioSrc) {
      try {

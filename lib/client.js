  ...
  factory: (require) => {
    ...
    function startAssistantVoiceAutoPlay() {
      ...
      const scanAndPlay = () => {
        ...
        // b) 关键：只在"新语音到达"（未播数比上次扫描多）时才播，且只播最后一条。
        //    历史积压（重开页面第一次扫描）不自动播——由用户手动点或标记为已读。
        if (unplayed.length === 0) { lastSeenVoiceCount = 0; return; }
        if (lastSeenVoiceCount === 0 && unplayed.length > 1) {
          // 首扫就发现多条未播 = 历史积压：全部标记已播（不播），只保留最新的待播？
          // 保守策略：积压语音一律标记已播，只有此后新到的才自动播。
          for (const card of unplayed.slice(0, -1)) {
            const vid = card.getAttribute("data-voice-id");
            if (vid) { playedVoiceIds.add(vid); persistPlayedVoiceIds(); }
          }
          lastSeenVoiceCount = 1;
          return;
        }
        lastSeenVoiceCount = unplayed.length;
        const target = unplayed[unplayed.length - 1]; // 只播最新一条
    ...
    function ToolbarLeft({ connection, sessionId }) {
      ...
      const sendVoiceBlob = useCallback(async (blob, durationMs) => {
        ...
        const sendAsVoice = async (images) => {
          ...
          // 写 sha256 对象池并返回 ref），再发 voice 引用块——0.1.5 session/prompt 已支持。
          ...
          // 语音气泡注入器数据源（降级路径同款）：/api/voice 读对象池
          pendingVoiceQueue.push({ voiceId: sv.voiceId, ext: sv.ext ?? "webm", objectRef: sv.ref });
          return await rpcPrompt([{ type: "voice", attachment: { ...sv.ref, ...(durationMs === undefined ? {} : { durationMs }) } }, ...images]);
        };
        try {
          ...
          // [2026-09-11] 直发失败降级：录音已在 sendAsVoice 里落对象池并入注入队列。
      ...
      const startRecording = useCallback(async () => {
        ...
        try {
          ...
          recorder.onstop = () => {
            ...
            if (blob.size > 0) void sendVoiceBlob(blob, secondsRef.current);
          };
          recorder.start();

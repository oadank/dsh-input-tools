...
// 事件原生渲染）；rc.7 上 AI 语音音频已生成但界面不显示，属 rc.7 硬伤，引导用户使用源码版。
    ...
    function VisionSettingsSection() {
      ...
      return h("div", { style: { display: "flex", flexDirection: "column", gap: "14px", padding: "16px", width: "100%", boxSizing: "border-box" } },
        ...
        // [2026-08-22] 直接可见的说明（不再用 ? 悬浮）
        h("div", { style: { fontSize: "12px", lineHeight: 1.8, color: "var(--dsw-alias-label-secondary,#9aa3ad)", background: "rgba(128,128,128,.05)", border: "1px solid var(--dsw-alias-border-l1,#333a45)", borderRadius: "8px", padding: "8px 12px" } },
          "识图工具 look_image 三种模式：describe=看图描述（默认，一两句）；reverse=像素级反推生图提示词；text=逐字提取图中文字。AI 收到图片后按提问自动选择。",
          h("div", {}, "配置「本地」= 本机起的 OpenAI 兼容 /v1 端点（如 ollama 11434/v1，无需 Key）；「在线」= 云端 API（填地址 + API Key）。下方可测试配置连通、查看/编辑各模式提示词。"),
        ),
        // 配置卡片

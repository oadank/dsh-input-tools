...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function injectVoiceCard(anchorEl, voiceId, ext) {
      try {
        // [2026-08-21 修] 不再往上爬找"消息气泡"（dsh class 是 CSS 模块 hash，匹配不可靠，
        // 会爬到消息列表/页面级容器导致语音条跑到网页底部且被拉伸成全宽横条）。
        // 直接插到文本块的父容器末尾（紧跟文字），宽度自适应不撑满
        ...
        card.style.cssText = "display:inline-flex;align-items:center;gap:8px;background:rgba(229,72,77,.1);border:1px solid rgba(229,72,77,.28);border-radius:999px;padding:4px 12px;margin-top:6px;width:fit-content;max-width:260px;";
        card.append(btn, dur);

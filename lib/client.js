...
// 事件原生渲染）；rc.7 上 AI 语音音频已生成但界面不显示，属 rc.7 硬伤，引导用户使用源码版。
    ...
    function startImageCardObserver() {
      ...
      const tryInject = () => {
        ...
        for (const el of els) {
          ...
          if (el.querySelector("img[data-img-card]")) { injectedImgEls.add(el); continue; }
          const text = el.textContent ?? "";
          ...
          const m = IMG_MARK_RE.exec(text);
          IMG_MARK_RE.lastIndex = 0;
          if (!m) { injectedImgEls.add(el); continue; }
          const path = (m[1] ?? "").trim();
          if (path === "") { injectedImgEls.add(el); continue; }
          injectImageCard(el, path);
    ...
    function injectImageCard(anchorEl, path) { try {
        const text = anchorEl.textContent ?? "";
        const stripped = text.replace(/\[图片\][^\[]*?\[\/图片\]/g, "").trim();
        if (stripped === "" && anchorEl.children.length === 0) anchorEl.textContent = "";
        else if (stripped !== text) anchorEl.textContent = stripped;
        const chip = document.createElement("span");
        chip.setAttribute("data-img-chip", "1");
        chip.style.cssText = "display:inline-flex;align-items:center;gap:8px;margin:4px 6px;padding:6px 12px 6px 6px;border-radius:14px;background:rgba(91,156,255,.12);border:1px solid rgba(91,156,255,.32);color:var(--dsw-alias-link,#5b9cff);font-size:13px;cursor:zoom-in;user-select:none;vertical-align:middle;max-width:320px;";
        ...
        label.style.cssText = "overflow:hidden;text-overflow:ellipsis;white-space:nowrap;min-width:0;flex:1 1 auto;font-family:monospace;";
      ...
      return h("div", { style: { position: "relative", display: "inline-flex", alignItems: "center", gap: "10px" } },
        ...
        visionDraftPath !== null ? h("span", { style: { position: "relative", display: "inline-flex" } },
          h("img", {
            ...
            alt: "已选图片",
            title: "已选图片（路径已插入输入框，直接发送即可）",
            style: { width: "36px", height: "36px", objectFit: "cover", borderRadius: "6px", border: "1px solid var(--dsw-alias-border-l1,#333a45)", cursor: "default" },
          }),
          h("button", {
            ...
            onClick: () => setVisionDraftPath(null),
          }, "✕"),
        ) : null,

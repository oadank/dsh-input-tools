                ...
                ui: {
                  ...
                  options: async (session) => {
                    const r = await fetch("/copilot-analyze", {
                      method: "POST",
                      headers: { "Content-Type": "application/json" },
                      body: JSON.stringify({ sessionId: String((session && session.sessionId) || ""), contextTurns: 24 }),
                    });
                    const j = await r.json().catch(() => null);
                    if (!r.ok || !j || j.ok === false) {
                      throw new Error((j && (j.error || (j.probe && j.probe.reason))) || ("副驾调用失败 HTTP " + r.status));
                    }
                    ...
                    return list.map((c, i) => {
                      ...
                      return {
                        id: t,
                        label: t.length > 46 ? t.slice(0, 46) + "…" : t,
                        detail: "拟用 " + (Number.isFinite(pv) ? Math.round(pv * 100) + "%" : "—")
                          + (j.judge && j.judge.risk ? "｜风险 " + j.judge.risk : "")
                          + (t.length > 46 ? "｜完整内容选中后填入输入框" : ""),
                        badge: i === 0 && Number.isFinite(pv) && pv >= 0.6 ? "荐" : undefined,
                        active: i === 0,
                      };
                    });
                  },

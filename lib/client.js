...
// 事件原生渲染）；rc.7 上 AI 语音音频已生成但界面不显示，属 rc.7 硬伤，引导用户使用源码版。
    ...
    function retroScan() {
      ...
      if (!RETRO.domReported && sessionId !== undefined) {
        ...
        void retroApi("/retro-dom", { sessionId, dump: [{ ver: "probe-3", sid: String(sessionId ?? "none").slice(-13), rows: dump.length, keys: dump, gone: [...gone].slice(0, 6), ids: [...goneIds].map((x) => String(x).slice(0, 18)).slice(0, 5), txt: goneTexts.length, turns: [...deadTurns].slice(0, 6) }] })
      }
    ...
    function apply(ctx) {
      ...
      ctx.effect(() => {
        ...
        const disposers = [
          ...
          // [2026-09-25 老大定稿] 「决策模型」独立分区：只从预设里选（默认阿里云百炼已选好），
          // 不让人填地址密钥；带「测一下」当场验真，切完实时生效。
          safeInject("settings.section", () => ctx.slots.register({
            name: "settings.section",
            id: "decision-model",
            order: 10,

    ...
    function VoiceSettingsSection() {
      ...
      const pickVdGroup = (g) => {
        if (g === "examples") {
          ...
          // 切到设计音色时若描述为空，自动填入内置默认（自己编写的，与官方示例不同）
          if ((eng.voicedesign?.context ?? "").trim() === "") {
            setEngine("voicedesign", { mode: "custom", context: CUSTOM_VOICE_DEFAULT, emotion: false }, true);
          } else {
            pickVdMode("custom");
          }
        } else {
            ...
            h("div", { style: { display: "flex", flexDirection: "column", gap: "8px", borderTop: "1px dashed var(--dsw-alias-border-l1,#333a45)", borderLeft: "2px solid #38bdf8", paddingTop: "8px", paddingLeft: "8px" } },
              ...
              vdGroup === "examples" ? h("div", { style: { display: "flex", flexDirection: "column", gap: "6px" } },
                ...
                h("div", { style: { display: "flex", alignItems: "center", gap: "8px" } },
                  ...
                  h("span", { style: { fontSize: "11.5px", color: "var(--dsw-alias-label-secondary,#9aa3ad)" } }, "每次随机一条描述，质感按锁定年龄分档取样"),
                ),
              ),
            ),

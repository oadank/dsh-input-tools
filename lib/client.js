...
// 事件原生渲染）；rc.7 上 AI 语音音频已生成但界面不显示，属 rc.7 硬伤，引导用户使用源码版。
     ...
     * ① aria-selected 的选中行；② aria-current 的选中行；③ 页面上只有一行带 data-session-id 时用它；
     * ④ 地址栏里的 session-<uuid>（手机 PWA、侧栏收起时唯一可靠的来源）。
     */
    ...
    function retroAsk({ title, quote, note, more, stopOption, replyOption }) {
      return new Promise((resolve) => {
        ...
        if (more !== undefined && more !== "") {
          ...
          sum.textContent = "为什么"
          sum.style.cssText = "font-size:11.5px;color:var(--dsw-alias-label-secondary,#9aa3ad);cursor:pointer;margin:-2px 0 6px;list-style-position:inside"
          const dp = document.createElement("p"); dp.className = "dsh-retro-note"; dp.style.marginTop = "2px"; dp.textContent = more
          det.append(sum, dp)
        ...
        if (stopOption) {
          ...
          const sp = document.createElement("span"); sp.textContent = "同时叫停本轮回答"
          lab.append(cb, sp)
        ...
        if (replyOption) {
          ...
          const sp2 = document.createElement("span"); sp2.textContent = "连它的回话一起删"
          lab2.append(cb2, sp2)
    ...
    const RETRO_STAGES = {
      queue: {
        title: "抽回这条？",
        note: "模型还没看到这条，抽回就当没发过。",
        more: "它还排在队列里，一个字都没送出去。抽走不留痕迹，只动这一条。",
        stop: false,
      },
      running: {
        title: "撤回这条消息？",
        note: "这句模型已经看到了。删它，往后它再也看不到。",
        more: "字已经计过钱，删不掉已发生的部分；本轮正在跑的回答可以顺手叫停。已经让它办的事（改文件、发消息）不会因此撤销。默认只删你这一条，模型这轮答出去的内容保留。",
        stop: true,
      },
      done: {
        title: "撤回这条消息？",
        note: "这段聊完了。删它 = 以后每次提问都看不到这句。",
        more: "界面上这条一并消失。当时让它办出去的事不会因此撤销。默认只删你这一条，模型当时的回话保留。",
        stop: false,
      },
      compacted: {
        title: "撤回这条消息？",
      ...
      // 所以"模型看不到"这个结论成立，措辞改成下面这样，不再谎称一定是折叠。
        note: "这条早被摘要顶掉了，模型手里没原文。",
        more: "模型那边只剩几段摘要，这条的原文早就不带着了。删它等于顺手把界面这一行也清掉，只删你这一条。",
        stop: false,
        reply: true,
      },
      ...
      windowed: {
        title: "撤回这条消息？",
        note: "这条这次没带进模型上下文，删了不影响它。",
        more: "太久远，这次只带了最近若干条。所以现在删它对模型没影响；删掉是让它以后也不会被重新带回去。只删你这一条。",
        stop: false,
        reply: true,
      },
      'not-message': {
        title: "撤回这条消息？",
        note: "认不出它的状态，删它只收界面这一行。",
        more: "它不在模型带的上下文里，服务端也没按一条消息认到它。删它会把界面这一行收掉，之后模型也不会再看到它。",
        stop: false,
      },
      ...
      mismatch: {
        title: "没敢删",
        note: "对不上号，没敢删。刷新再试。",
        more: "这一行的文字和会话里对上的记录不一致，说明我认错了行。为防删错别条，这次什么都没动。",
        stop: false,
      },
      retracted: {
        title: "这条已经撤回过了",
        note: "这条已经撤回过了，再点没后果。",
        more: "模型已经看不到它，界面上也该收着。再点一次不会有别的后果。",
        stop: false,
      },
      unknown: {
        title: "撤回这条消息？",
        note: "阶段认不准。删它 = 以后它看不到这句。",
        more: "认不出这条现在处于哪个阶段。删它会把这句从模型往后的记忆里拿掉、界面一并消失；已经办出去的事不会撤销。",
        stop: true,
      },
    }
    ...
    async function retroDeleteMessage(sessionId, addr, preview) {
      ...
      const ans = await retroAsk({ title: stage.title, quote: preview, note: stage.note, more: stage.more, stopOption: stage.stop, replyOption: stage.reply })
      if (!ans.ok) return
    ...
    function retroScan() {
      ...
      for (const row of document.querySelectorAll("[data-chat-flow-kind='user'],[data-chat-flow-kind='steering']")) {
        ...
        retroMountBtn(seat, "msg-" + rowId, "撤回这条", () => {
          ...
          if (sid === undefined) { retroToast("认不出这是哪段会话（把左侧历史列表展开再点）", true); return }
          void retroDeleteMessage(sid, addr, text)
        })
      }
      ...
      for (const host of document.querySelectorAll("[data-pending-id]")) {
        ...
        retroMountBtn(seat, "pend-" + mid, "抽回这条（模型还没看到）", () => {
          const sid = retroCurrentSessionId()
          if (sid === undefined) { retroToast("认不出这是哪段会话（把左侧历史列表展开再点）", true); return }
          void retroDeleteMessage(sid, { messageId: mid }, (host.innerText ?? "").slice(0, 120))
        })
      }
